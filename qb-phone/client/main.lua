...
local function LoadPhone()
    ...
    QBCore.Functions.TriggerCallback('i13-phone:server:GetPhoneData', function(pData)
        ...
        if pData.InstalledApps ~= nil and next(pData.InstalledApps) ~= nil then
            for k, v in pairs(pData.InstalledApps) do
                ...
                Config.PhoneApplications[v.app] = {
                    ...
                    requiredItem = AppData.requiredItem,
                    slot = AppData.slot,
                    Alerts = 0,
                }
            end
        ...
        if pData.Jobs ~= nil and next(pData.Jobs) ~= nil then
            PhoneData.Jobs = pData.Jobs
        end
...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('i13-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.Notify("Sul ei ole telefoni", "error")
        end
    end)
end
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Telefon",
                text = "Sul ei ole ühtegi sissetulevat kõne",
                icon = "fas fa-phone",
                color = "rgb(10, 180, 100)",
            },
        })
...
RegisterNUICallback('GetJobs', function(data, cb)
    if PhoneData.Jobs ~= nil and next(PhoneData.Jobs) ~= nil then
        cb(PhoneData.Jobs)
    else
        cb(nil)
    end
...
RegisterNUICallback('GetCurrentJob', function(data, cb)
...
RegisterNUICallback('GetCurrentDuty', function(data, cb)
    local Playerduty = QBCore.Functions.GetPlayerData().job.onduty
    if Playerduty ~= nil then
        cb(Playerduty)
    else
        cb(nil)
    end
...
RegisterNUICallback('activatekeys', function(data, cb)
    local veh = data.veh
    if veh.plate ~= nil then
        QBCore.Functions.Notify('Võtmed Aktiveeritud', 'success')
        TriggerEvent('vehiclekeys:client:SetOwner', veh.plate)
        SetVehicleDoorsLocked(veh, 1)
    else
        QBCore.Functions.Notify('See sõiduk ei kuulu sulle või ei ole olemas', 'error')
    end
end)
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    QBCore.Functions.Notify('GPS Asukoht märgitud: '..data.alert.title)
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('i13-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Sa oled liiga kaugel. GPS määratud race\'ile.', 'error', 5000)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("GPS määratud: " .. data.HouseData.HouseData.adress .. "!", "success")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS määratud!', 'success')
end)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        QBCore.Functions.TriggerCallback('i13-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
                ...
                SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Bank", text = "Kontot ei ole olemas!", icon = "fas fa-university", color = "#ff0000", }, })
...
RegisterNetEvent('i13-phone:client:removeJobFromTable', function(selectedJob)
    for k,v in pairs(PhoneData.Jobs) do
        if v.job == selectedJob then
            ...
            -- print('removed '..selectedJob..' from jobs list')
        end
    end
end)
    ...
    if not delete then -- New Tweet
        if src ~= MyPlayerId then
            SendNUIMessage({
                action = "PhoneNotification",
                PhoneNotify = {
                    title = "Uus Tweet (@"..NewTweetData.firstName.." "..NewTweetData.lastName..")",
...
RegisterNetEvent('i13-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Mail",
            text = "Sa said uue maili: "..MailData.sender,
...
RegisterNetEvent('i13-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Advertisement",
            text = "Uus kuulutus postitatud: "..LastAd,
            icon = "fas fa-ad",
            color = "#ff8f1a",
            timeout = 2500,
        },
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Messenger",
                        text = "Uus sõnum numbrilt "..IsNumberInContacts(SenderNumber).."!",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 1500,
                    },
...
RegisterNetEvent('i13-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Pank",
                text = "$"..amount.." eemaldatud sinu kontolt!", 
                icon = "fas fa-university", 
                color = "#badc58",
                timeout = 3500,
            },
...
RegisterNetEvent('i13-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Telefon",
            text = "Sul on uus soovitatud kontakt!",
            icon = "fa fa-phone-alt",
            color = "#04b543",
            timeout = 1500,
        },
