                    ...
                    if not isBusy then
                        if not isKidnapped then
                            -- if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                QBCore.Functions.Notify('Sa oled pagasiruumis', 'success', 4000)
                                TrunkCam(true)
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                QBCore.Functions.Notify('Pagasiruum on suletud?', 'error', 2500)
                            end
                        end
                    else
                        QBCore.Functions.Notify('Keegi on juba ees', 'error', 2500)
                    end 
                else
                    QBCore.Functions.Notify('Sa oled juba pagasiruumis', 'error', 2500)
                end 
            else
                QBCore.Functions.Notify('Sa ei saa siia minna', 'error', 2500)
            end
        end, plate)
    else
        QBCore.Functions.Notify('Sa ei saa siia minna', 'error', 2500)
...
AddEventHandler('qb-trunk:client:GetIn', function(isKidnapped)
    ...
    if closestVehicle ~= 0 then
                    ...
                    if not inTrunk then
                        if not isBusy then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                Wait(500)
                                SetVehicleDoorShut(closestVehicle, 5, false)
                                -- QBCore.Functions.Notify('Sa oled juba pagasiruumis', 'Goodluck', 4000)
                                TrunkCam(true)
                            else
                                QBCore.Functions.Notify('Pagasiruum on suletud?', 'error', 2500)
                            end
        ...
        QBCore.Functions.Notify('Sa ei ole auto juures', 'error', 2500)
    end
end)
        ...
        if inTrunk then
            if not isKidnapped then
                ...
                if DoesEntityExist(vehicle) then
                    DrawText3Ds(drawPos.x, drawPos.y, drawPos.z + 0.75, '[E] Välju Pagasiruumist')
