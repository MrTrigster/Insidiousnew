...
RegisterNetEvent('hospital:client:UseIfak', function()
    ...
    QBCore.Functions.Progressbar("use_bandage", "Kasutab IFAK'i...", 4000, false, true, {
...
RegisterNetEvent('hospital:client:UseBandage', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar("use_bandage", "Kasutab sidemeid...", 4000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
        ...
        RemoveBleed(1)
        QBCore.Functions.Notify("Peatasid verejooksu", "success")
...
RegisterNetEvent('hospital:client:UsePainkillers', function()
    local ped = PlayerPedId()
    QBCore.Functions.Progressbar("use_bandage", "Võtab valuvaigisteid...", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
...
CreateThread(function()
    while true do
        if ifakHealing then
            ...
            SetEntityHealth(ped, currentHealth + math.random(4, 8))
        end
        Wait(0)
    end
end)
