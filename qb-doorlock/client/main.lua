...
local function lockpickFinish(success)
	if success then
		QBCore.Functions.Notify('Uks Avatud', 'success', 2500)
...
RegisterNetEvent('qb-doorlock:client:addNewDoor', function()
	...
	local dialog = exports['qb-input']:ShowInput({
		...
		inputs = {
			...
			},
			{
				...
				type = "select",
				options = {
					{ value = "door", text = "Single Door" },
					{ value = "double", text = "Double Door" },
					{ value = "sliding", text = "Single Sliding Door" },
					{ value = "doublesliding", text = "Double Sliding Door" },
					{ value = "garage", text = "Garage" }
				},
				default = Config.SaveDoorDialog and doorData.doortype,
			},
			{
				text = "Job Authorisation",
				name = "job",
				type = "text",
				isRequired = false,
			...
			},
			{
				...
				type = "checkbox",
				options = {
					{ value = "locked", text = "Locked", checked = (Config.SaveDoorDialog and doorData.locked) },
					{ value = "pickable", text = "Lockpickable", checked = (Config.SaveDoorDialog and doorData.pickable == 'true') },
					{ value = "cantunlock", text = 'Cant Unlock', checked = (Config.SaveDoorDialog and doorData.cantunlock == 'true') },
					{ value = "hidelabel", text = 'Hide Door Label', checked = (Config.SaveDoorDialog and doorData.hidelabel == 'true') },
				}
			}
		}
	})
	...
	if Config.DoorList[identifier] then 
		QBCore.Functions.Notify("A door with this identifier already exists in the config. "..identifier, 'error') 
		canContinue = true 
		return 
	end
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		if not model or model == 0 then QBCore.Functions.Notify("Did not receive a model hash, if the door is transparent, make sure you aim at the frame of the door", 'error') canContinue = true return end
		result = DoorSystemFindExistingDoor(coords.x, coords.y, coords.z, model)
		if result then QBCore.Functions.Notify("This door is already registered", 'error') canContinue = true return end
		...
		if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then QBCore.Functions.Notify("Did not receive a model hash, if the door is transparent, make sure you aim at the frame of the door", 'error') return end
		if entity[1] == entity[2] then QBCore.Functions.Notify("Both doors can't be the same entity", 'error') canContinue = true return end
		doorData.doorHash = {}
		for i = 1, 2 do
