                ...
                if (status.CanCall) {
                    if (!status.InCall) {
                        if (QB.Phone.Data.AnonymousCall) {
                            QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Sa alustasid anonüümse kõne!");
                        }
                        ...
                        $(".quake-outgoing-icon-container").css({"background":"rgb(10, 180, 100)"});
                        $(".quake-outgoing-icon").html('<i class="fas fa-phone"></i>');
                        $(".quake-outgoing-title").css({"color":"#eee"});
                        if (!QB.Phone.Data.IsOpen) {
                            QB.Phone.Animations.BottomSlideUp('.container', 300, -55);
                        }
                        var label = cData.name
                        QB.Phone.Animations.TopSlideDown(".quake-outgoing-container", 200, 5);
                        $(".quake-outgoing-title").html(label);
                        $(".quake-outgoing-text").html('00:00:00');
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Sa oled juba kõnes!");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "See isik on hõivatud!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "See isik ei ole saadaval!");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "Sa ei saa helistada endale!");
        }
    });
});
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            if (contact.name == '') contact.name = 'Hmm, ma ei peaks saama seda teha...'
        ...
        $("#total-contacts").text("0 kontakti");
    }
};
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
        ...
        QB.Phone.Notifications.Add("fa fa-phone-alt", "Telefon", "Sa ei saa endale sõnumit saata.", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    if (CurrentEditContactData.name == '') CurrentEditContactData.name = 'Hmm, ma ei peaks saama seda teha...'
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName == '') ContactName = 'Hmm, ma ei peaks saama seda teha...'
    ...
    if (ContactName != "" && ContactNumber != "") {
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Telefon", "Täida kõik lüngad!");
    }
});
        ...
        if (cData.number !== QB.Phone.Data.PlayerData.charinfo.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "See isik on juba kõnes!");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefon", "See isik ei ole saadaval!");
            }
...
CancelOutgoingCall = function() {
    ...
    //     QB.Phone.Data.currentApplication = null;
    // }
    QB.Phone.Animations.TopSlideDown(".quake-outgoing-container", 200, -8);
    setTimeout(function(){
        $(".quake-outgoing-container").css({"display":"none"});
    }, 400)
    QB.Phone.Data.CallActive = false;
    QB.Phone.Data.currentApplication = null;
}
...
$(document).on('click', '#quake-outgoing-decline', function(e){
    e.preventDefault();
...
$(document).on('click', '#quake-decline', function(e){
    e.preventDefault();
...
$(document).on('click', '#phone-currentcall-decline', function(e){
    e.preventDefault();
...
IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            ...
            QB.Phone.Animations.TopSlideUp('.'+QB.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                var Label = "Sul on sissetulev kõne "+CallData.name
                if (AnonymousCall) {
                    Label = "Sulle helistab Anonüümne helistaja"
                }
                $(".call-notifications-title").html("Sissetulev Kõne");
...
IncomingCallQuake = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!QB.Phone.Data.CallActive) {
            $(".quake-notification-icon-container").css({"background":"rgb(10, 180, 100)"});
            $(".quake-notification-title").css({"color":"#eee"});
            if (!QB.Phone.Data.IsOpen) {
                QB.Phone.Animations.BottomSlideUp('.container', 300, -55);
            }
            var label = CallData.name
            if (AnonymousCall) {
                label = 'Anonüümne'
            }
            $(".quake-notification-icon").html('<i class="fas fa-phone"></i>');
            $(".quake-notification-title").html(label);
            $(".quake-notification-text").html('00:00:00');
            QB.Phone.Animations.TopSlideDown(".quake-notification-container", 200, 5);

            QB.Phone.Data.CallActive = true;
        }
    } else {
        QB.Phone.Animations.TopSlideUp(".quake-notification-container", 200, -8);
        if (!QB.Phone.Data.IsOpen) {
...
//             $(".phone-currentcall-title").html("Sissetulev kõne");
//         } else if (cData.CallType == "outgoing") {
//             $(".phone-currentcall-title").html("Väljaminev kõne");
//         } else if (cData.CallType == "ongoing") {
//             $(".phone-currentcall-title").html("Kõnes ("+cData.CallTime+")");
//         }
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        if (cData.CallType == "incoming") {
            ...
            var label = cData.TargetData.name
            if (cData.TargetData.name === '') {
                label = cData.TargetData.number
            }
            $(".quake-notification-icon").html('<i class="fas fa-phone"></i>');
            $(".quake-notification-title").html(label);
            $(".quake-notification-text").html('00:00:00');
        ...
        $(".phone-currentcall-contact").html(cData.CallTime);
    } else {
        QB.Phone.Animations.TopSlideDown(".phone-currentcall-container", 200, -8);
        setTimeout(function(){
            $(".phone-currentcall-container").css({"display":"none"});
        }, 400)
        if (!QB.Phone.Data.IsOpen) {
...
// $(document).on('click', '.phone-currentcall-icon', function(e){
//     e.preventDefault();
...
$(document).on('click', '#quake-answer', function(e){
    e.preventDefault();
...
QB.Phone.Functions.AnswerCall = function(CallData) {
    ...
    $(".quake-notification-container").css({"display":"none"});
    $(".quake-outgoing-container").css({"display":"none"});
    QB.Phone.Animations.TopSlideDown(".quake-outgoing-container", 200, -8);

    $(".phone-currentcall-container").css({"display":"block"});
    QB.Phone.Animations.TopSlideDown(".phone-currentcall-container", 200, 5);
...
QB.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
    ...
    if (AmountOfSuggestions > 0) {
        $(".amount-of-suggested-contacts").html(AmountOfSuggestions + " kontakti");
        ...
        $(".amount-of-suggested-contacts").html("0 kontakti");
    }
}
