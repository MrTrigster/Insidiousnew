        ...
        if CurrentCops >= Config.MinimumDrugSalePolice then
            if result ~= nil then
                ...
                if not cornerselling then
                    ...
                    QBCore.Functions.Notify('Alustasid narko müügiga')
                    startLocation = GetEntityCoords(PlayerPedId())
                else
                    cornerselling = false
                    LocalPlayer.state:set("inv_busy", false, true)
                    QBCore.Functions.Notify('Lõpetasid narko müügi')
                end
            else
                QBCore.Functions.Notify('Sul ei ole narkot', 'error')
                LocalPlayer.state:set("inv_busy", false, true)
            end
        else
            QBCore.Functions.Notify("Linnas pole piisavalt politseinikke (".. Config.MinimumDrugSalePolice .." Vaja)", "error")
        end
    end)
end)
...
local function toFarAway()
    QBCore.Functions.Notify('Liikusid liiga kaugele', 'error')
    LocalPlayer.state:set("inv_busy", false, true)
    cornerselling = false
    hasTarget = false
    startLocation = nil
...
local function callPolice(coords)
    ...
    AddTextComponentString("Narkotehing")
...
local function SellToPed(ped)
    local title = 'Võimalik Narkotehing'
    local pCoords = GetEntityCoords(PlayerPedId())
    hasTarget = true
            ...
            if getRobbed == 18 or getRobbed == 9 then
                ...
                if pedDist < 1.5 and cornerselling then
                    ...
                    if IsControlJustPressed(0, 47) then
                        QBCore.Functions.Notify('Pakkumine tühistatud', 'error')
                        hasTarget = false
        ...
        if stealingPed ~= nil and stealData ~= nil then
            ...
            if IsEntityDead(stealingPed) then
                ...
                if #(pos - pedpos) < 1.5 then
                    DrawText3D(pedpos.x, pedpos.y, pedpos.z, "[E] Korja üles")
    ...
    while true do
        ...
        if cornerselling then
            ...
            if not hasTarget then
                ...
                if closestDistance < 15.0 and closestPed ~= 0 and not IsPedInAnyVehicle(closestPed) and not IsPedInAnyVehicle(player) and not IsEntityAMissionEntity(closestPed) and GetPedType(closestPed) ~= 28 then
