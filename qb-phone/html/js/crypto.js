...
function SetupCryptoData(Crypto) {
    ...
    CryptoData.Portfolio = Crypto.Portfolio; //(Crypto.Portfolio).toFixed(6);
    CryptoData.Worth = Crypto.Worth;
    CryptoData.WalletId = Crypto.WalletId;

    $(".crypto-action-page-wallet").html("Portfoolio: "+CryptoData.Portfolio+" INC");
    ...
    if (CryptoData.History.length > 0) {
        ...
        $.each(CryptoData.History, function(i, change){
            ...
            var Element =   '<div class="cryptotab-course-block">' +
                                ...
                                '<span class="cryptotab-course-block-title">Kursi Muutus</span>' +
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (QB.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://qb-phone/BuyCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "Pank", "&#36; "+Price+",- on eemaldatud Sinu kontolt!", "#badc58", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Sul ei ole piisavalt raha..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Sul ei ole piisavalt raha..", "#badc58", 1500);
        ...
        QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Täida kõik lüngad!", "#badc58", 1500);
    }
});
        ...
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://qb-phone/SellCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    QB.Phone.Data.PlayerData.money.bank = parseInt(QB.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    QB.Phone.Notifications.Add("fas fa-university", "Pank", "&#36; "+Price+",- on lisatud Sinu kontole!", "#badc58", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Sul ei ole piisavalt INC..", "#badc58", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Sul ei ole piisavalt INC..", "#badc58", 1500);
            ...
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://qb-phone/TransferCrypto', JSON.stringify({
                    ...
                    if (CryptoData == "notenough") {
                        ...
                        QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Seda Portfoolio-ID ei ole olemas!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Sa kandsid "+Coins+",- kontole "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-university", "Crypto", "Sa ei saa endale kanda..", "#badc58", 2500);
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Sul ei ole piisavalt INC..", "#badc58", 1500);
        }
...
$(".crypto-action-page-buy-crypto-input-coins").keyup(function(){
    ...
    $(".crypto-action-page-buy-crypto-input-money").html(MoneyAmount+" Dollarit");
});
...
$(".crypto-action-page-sell-crypto-input-coins").keyup(function(){
    ...
    $(".crypto-action-page-sell-crypto-input-money").html(MoneyAmount+" Dollarit");
});