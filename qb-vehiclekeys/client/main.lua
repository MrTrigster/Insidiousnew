                ...
                if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                    if IsEntityDead(driver) then
                        if not isTakingKeys then
                            ...
                            QBCore.Functions.Progressbar("steal_keys", "Võtab isikult võtmed...", 2500, false, false, {
    ...
    if targetVehicle then
        if id ~= nil then
            ...
            if HasKeys(targetPlate) then
                ...
                QBCore.Functions.Notify("Sul ei ole selle sõiduki võtmeid", 'error')
            end
        else
        ...
        if vehicle ~= nil and vehicle ~= 0 then
            if GetPedInVehicleSeat(vehicle, -1) == ped then
                ...
                if usingAdvanced then
					...
					if success then
                        local seconds2 = math.random(45, 47)
                        local circles2 = math.random(1, 2)
					...
					local success = exports['qb-lock']:StartLockPickCircle(circles, seconds, success)
					if success then
                        local seconds2 = math.random(55, 70)
                        local circles2 = math.random(1, 2)
                        local success2 = exports['qb-lock']:StartLockPickCircle(circles2, seconds2, success2)
                        if success2 then
						    StopAnimTask(ped, dict, "machinic_loop_mechandplayer", 1.0)
...
function Hotwire(vehicle, plate)
    ...
    QBCore.Functions.Progressbar("hotwire_vehicle", "Ühendab juhtmeid...", hotwireTime, false, true, {
...
function CarjackVehicle(target)
    ...
    QBCore.Functions.Progressbar("rob_keys", "Üritab sõidukit röövida...", Config.CarjackingTime, false, true, {}, {}, {}, {}, function()
...
function PoliceCall(plate)
    local chance = math.random(1, 100)
    if chance <= 40 then
        if plate then
            local data = {displayCode = '10-75', description = 'Autovargus', isImportant = 1, recipientList = {'police'}, length = '10000', infoM = 'fa-info-circle', info = 'Autovarguse katse'}
            local dispatchData = {dispatchData = data, caller = 'Auto Alarm', coords = GetEntityCoords(PlayerPedId())}
...
function PoliceLocalCall()
    local data = {displayCode = '10-75', description = 'Autovargus', isImportant = 1, recipientList = {'police'}, length = '10000', infoM = 'fa-info-circle', info = 'Kohaliku auto varguse katse'}
    local dispatchData = {dispatchData = data, caller = 'Kohalik', coords = GetEntityCoords(PlayerPedId())}
    TriggerServerEvent('wf-alerts:svNotify', dispatchData)
end
