...
TryToFish = function()
    QBCore.Functions.TriggerCallback('qb-fishing:GetItemData', function(count)
        ...
        if count ~= nil then
            if count == 0 then
                QBCore.Functions.Notify("Sul on vaja õnge ja sööta, et kala püüda", "error")
            else
                ...
                if waterValidated then
                    ...
                    QBCore.Functions.Notify("Pead vett vaatama, et kala püüda", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        QBCore.Functions.Notify('Sul ei ole sööta', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        ShowHelpNotification("Heida õng vette ~INPUT_DETONATE~")

        if GetGameTimer() - startedCasting > 5000 then
            QBCore.Functions.Notify("Sa pead õnge vette heitma", "primary")
    ...
    QBCore.Functions.Notify("Ootab..", "success", "10000")
...
--         ShowHelpNotification("Vajuta ~INPUT_CONTEXT~ rohelises alas")
...
IsInWater = function()
    ...
    QBCore.Functions.Notify("Kontrollib kalastamise asukohta", "success", "3000")
