...
local StatusList = {
    ['fight'] = 'Verised nukid',
    ['widepupils'] = 'Suured pupillid',
    ['redeyes'] = 'Punased silmad',
    ['weedsmell'] = 'Kanepihais',
    ['gunpowder'] = 'Püssiroht riietel',
    ['chemicals'] = 'Kemikaalide hais',
    ['heavybreath'] = 'Hingeldab',
    ['sweat'] = 'Higistab palju',
    ['handbleed'] = 'Veri kätel',
    ['confused'] = 'Segaduses',
    ['alcohol'] = 'Haiseb nagu alkohol',
    ['heavyalcohol'] = 'Tugev alkoholi hais'
}
...
RegisterNetEvent('evidence:client:ClearBlooddropsInArea', function()
    ...
    local blooddropList = {}
    QBCore.Functions.Progressbar('clear_blooddrops', 'Eemaldab vereplekke...', 5000, false, true, {
        ...
        if Blooddrops and next(Blooddrops) then
            ...
            QBCore.Functions.Notify('Vereplekid eemaldatud')
        end
    end, function() -- Cancel
...
RegisterNetEvent('evidence:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
        type = weapon,
        serie = serie and serie or 'Seeria number pole näha',
        coords = {
            x = coords.x,
            y = coords.y,
            z = coords.z - 0.9
...
RegisterNetEvent('evidence:client:ClearCasingsInArea', function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    QBCore.Functions.Progressbar('clear_casings', 'Eemaldab hülsse...', 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        if Casings and next(Casings) then
            ...
            QBCore.Functions.Notify('Hülsid eemaldatud')
        end
    end, function() -- Cancel
        QBCore.Functions.Notify('Katkestatud', 'error')
    end)
    ...
    while true do
        ...
        if CurrentCasing and CurrentCasing ~= 0 then
            ...
            if #(pos -vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z)) < 1.5 then
                DrawText3D(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, '~g~G~w~ - Hülss ~b~#' .. Casings[CurrentCasing].type)
        ...
        if CurrentBlooddrop and CurrentBlooddrop ~= 0 then
            ...
            if #(pos - vector3(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y,
                ...
                DrawText3D(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y, Blooddrops[CurrentBlooddrop].coords.z, '~g~G~w~ - Veri ~b~#' .. DnaHash(Blooddrops[CurrentBlooddrop].citizenid))
                if IsControlJustReleased(0, 47) then
        ...
        if CurrentFingerprint and CurrentFingerprint ~= 0 then
            ...
            if #(pos - vector3(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y,
                ...
                DrawText3D(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y, Fingerprints[CurrentFingerprint].coords.z, '~g~G~w~ - Sõrmejälg ')
                if IsControlJustReleased(0, 47) then
