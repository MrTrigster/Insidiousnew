...
function getFormattedDate(date, prefomattedDate = false, hideYear = false) {
  ...
  if (prefomattedDate) {
    return `${prefomattedDate} kell ${hours}:${minutes}`;
  }

  if (hideYear) {
    return `${day}. ${month} kell ${hours}:${minutes}`;
  }
...
function timeAgo(dateParam) {
  ...
  if (seconds < 5) {
    ...
    return "Umbes minut tagasi";
  } else if (minutes < 60) {
    return `${minutes} Minutit tagasi`;
  } else if (isToday) {
    return getFormattedDate(date, "Täna");
  } else if (isYesterday) {
    return getFormattedDate(date, "Eile");
  } else if (isThisYear) {
...
$(document).ready(() => {
  ...
  $(".profile-items").on("click", ".profile-item", async function () {
    ...
    $(".manage-profile-editing-title").html(`Haldad hetkel profiili ${result["firstname"]} ${result["lastname"]}`);
    ...
    let licencesHTML = '<div style="color: #fff; text-align:center;">Pole Lube</div>';
    let tagsHTML = '<div style="color: #fff; text-align:center;">Pole Tagi</div>';
    let convHTML = '<div style="color: #fff; text-align:center;">Puhas Register ?</div>';
    let vehHTML = '<div style="color: #fff; text-align:center;">Pole Sõidukeid</div>';
    let galleryHTML = '<div style="color: #fff; text-align:center;">Pole Pilte</div>';
    let propertyHTML = '<div style="color: #fff; text-align:center;">Pole Kinnisvara</div>';
    ...
    if (canCreateBulletin == 0) {
      ...
      $(".bulletin-items-continer")
        .prepend(`<div class="bulletin-item" data-id="${canCreateBulletin}">
                ...
                <div class="bulletin-date">${MyName} - Praegu</div>
                </div>
            </div>`);
    } else {
    ...
    if (canSaveProfile == true) {
      ...
      setTimeout(() => {
        ...
        $(".tags-holder")
          .find("div")
          .each(function () {
            if ($(this).text() != "" && $(this).text() != "Pole Tagi") {
              tags.push($(this).text());
            }
        });
            ...
            .each(function (index) {
              if ($(this).data("id") == cid) {
                if ($(this).hasClass("green-tag")) {
                  ...
                  if ($(this).text() == "Tagaotsitav") {
                    warrant = true;
                  }
                  if ($(this).text() == "Protsessitud") {
                    processed = true;
                  }
                  if ($(this).text() == "Seotud") {
                    isassociated = true;
  ...
  $(".manage-incidents-title-holder").on(
    ...
    function () {
      let tempalte =
        "📝 Kokkuvõte:\n\n[Kirjuta siia kokkuvõte]\n\n🔪 Konfiskeeritud Relvad/Esemed:\n\n· [Kirjuta siia konfiskeeritud relvad/esemed]\n\n-----\n💸 Trahv:\n⌚ Karistus:\n-----";
      $("#manage-incidents-title-input").val(
        "Nimi - Karistus - " + $(".date").html()
      );
      ...
      $(".manage-incidents-title-holder").prepend(
        `
            <div class="manage-incidents-title">Halda Sündmust</div>
  ...
  $(".manage-bolos-title-holder").on(
    ...
    function () {
      ...
      if ($(".badge-logo").attr("src") == "img/ems_badge.png") {
        template =
          "Intensiivravi ruum #: [ # ]\n\nRaporti ID: [ Raporti ID ]\n\nKellaaeg: [ Kuupäev ja kellaaeg siia ]\n\nOpereeriti: [Jah/Ei]\n\nVigastused:\n - [ Kirjuta vigastused siia ]\n\n\nAdditional Attending:\n - [ List Any Other Staff Here ]\n\n\n🧑‍🤝‍🧑 Lähedaste kontaktandmed:\n - [ Nimi ja Number ]\n\n\nMärkmed:\n[Kirjuta lisa märkmed siia]";
      }
      $(".manage-bolos-editing-title").html(
        "Täidad hetkel uut BOLO"
      );
    ...
    function () {
      ...
      $(".incidents-ghost-holder").prepend(
        `
          ...
          this
        ).data("cid")}">Tagaotsitav</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(
          this
        ).data("cid")}">Süüdi</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(
          this
        ).data("cid")}">Protsessitud</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(
          this
        ).data("cid")}">Seotud</div>
                </div>
          ...
          "cid"
        )}"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="Sisesta trahvisumma..." value="0" class="fine-amount" data-id="${$(
          this
        ).data("cid")}" type="number"></div>
                <div class="manage-incidents-title-tag" data-id="${$(this).data(
          "cid"
          ...
          "cid"
        )}"><img src="https://i.imgur.com/9Xn6xXK.png"> <input placeholder="Sisesta kuude arv..." value="0" class="sentence-amount" data-id="${$(
          this
        ).data("cid")}" type="number"></div>
            </div>
            `
  ...
  $(".manage-reports-title-holder").on(
    ...
    function () {
      ...
      if ($(".badge-logo").attr("src") == "img/ems_badge.png") {
        template =
          "Saadetud Intensiivravile: [Jah/Ei]\n\nSündmuse Raport:\n[ Kirjelda, mis juhtus ja kes tegi mida sündmuskohal. Kirjuta mis sündmuskohal silma jäi ning mida tehti patsiendi aitamiseks. ]\n\n\nVigastused:\n- [ Pane kirja mis vigastus/vigastused olid ]\n\n\n💉 Kirurgiline Raport:\n[ Kirjuta, mida tehti opereerides. Märgi probleemid, mis esinesid ning mida leiti opereerides. Märgi kes osalesid operatsioonil ning mis ülesannet nad täitsid. Raporti lõppu kirjuta, milline oli patsiendi seisund pärast opereerimist ]\n\n\nKohalolijad:\n- [ Märgi siia kohalolijad ]\n\n\nAdministreeritud Ravimid:\n- [ Kirjuta siia, mida kasutati ravimiseks ]\n\n\nMärkmed:\n[ Kirjuta siia lisa märkmed ]";
      }
      $(".manage-reports-editing-title").html(
        "Täidad hetkel uut raportit"
      );
            ...
            `
                            <div class="profile-item" data-id="0">
                                ...
                                <div style="display: flex; flex-direction: column;">
                                    <div class="profile-item-title">Ei leitud ühtegi sellist sõidukit</div>
                                    </div>
                            ...
                            <div style="display: flex; flex-direction: column;">
                                <div class="dmv-item-title">${value.model}</div>
                                    <div class="dmv-tags">
                                        ...
                                        <div class="dmv-tag ${stolen}">Varastatud</div>
                                        <div class="dmv-tag ${codefive}">Kood 5</div>
                                    </div>
                                </div>
                                <div class="dmv-bottom-info">
                                    <div class="dmv-id">Numbrimärk: ${value.plate} · Omanik: ${value.owner}</div>
                                </div>
                            </div>
                        </div>
                    `;
        ...
        args = [
          {
            ...
            icon: "fas fa-times",
            text: "Eemalda Varastatud Staatus",
            info: plate,
            status: "",
          },
        ];
  ...
  $(".contextmenu").on("click", ".toggle-duty", function () {
    ...
    let currentStatus = $(`[data-id="${info}"]`)
      .find(".unit-status")
      .html();
    if (currentStatus == "10-41") {
      $(`[data-id="${info}"]`).find(".unit-status").html("10-42");
      $(`[data-id="${info}"]`)
        .find(".unit-status")
    ...
    } else if (currentStatus == "10-42") {
      $(`[data-id="${info}"]`).find(".unit-status").html("10-41");
      $(`[data-id="${info}"]`)
        .find(".unit-status")
  ...
  function JobColors(sentJob) {
    if (sentJob) {
      if (PoliceJobs[sentJob] !== undefined) {
        ...
        $(".bolo-nav-item").html("BOLO");
        $(".bolos-search-title").html("Bolo");
        $("#bolos-search-input").attr("placeholder", "Otsi Bolo...");
        $(".manage-bolos-title").html("Halda Bolo");
        $(".manage-bolos-editing-title").html(
          "Täidad hetkel uut BOLO"
        );
        $(".boloplate-title").html("Numbrimärk");
        $(".boloowner-title").html("Omanik");
        $(".boloindividual-title").html("Isik");
        $("#boloplate").attr("placeholder", "Lisa numbrimärk siia...");
        $("#bolodetail").attr(
          "placeholder",
          "Bolo kirjeldus..."
        );
        ...
        $(".quote-span").html(
          "OOTA, MA TEEN ARVE ÄRA"
        );
        $(".bolo-nav-item").html("Intensiivravi");
        $(".bolos-search-title").html("Intensiivravi Check-in'id");
        $("#bolos-search-input").attr(
          "placeholder",
          "Otsi Check-in..."
        );
        $(".manage-bolos-title").html("Halda Intensiivravi Check-in");
        $(".manage-bolos-editing-title").html(
          "Täidad uut intensiivravi Check-in'i"
        );
        $(".boloplate-title").html("Eeldatav Paranemine");
        $(".boloowner-title").html("Lähedase Kontakt");
        $(".boloindividual-title").html("Patsient");
        $("#boloplate").attr(
          "placeholder",
          "Sisesta paranemise aeg..."
        );
        $("#bolodetail").attr(
          "placeholder",
          "Sisesta Intensiivravi Check-in'i detailid siia..."
        );
        $("#boloowner").attr(
          "placeholder",
          "Sisesta lähedase kontakt siia..."
        );
        $("#boloindividual").attr(
          "placeholder",
          "Sisesta patsiendi nimi ja isikukood siia..."
        );
        $(".incidents-nav-item").hide();
        $(".dmv-nav-item").hide();
        $("#reports-officers-involved-tag-title").html("Seotud Meedikud");
        $(".dispatch-title-ofsomesort").html("Dispatch");
        $(".dispatch-comms-container").fadeIn(0);
        $(".manage-profile-name-input-1").attr("readonly", true);
        $(".manage-profile-name-input-2").attr("readonly", true);
  ...
  window.addEventListener("message", function (event) {
    ...
    if (eventData.type == "show") {
      ...
      activeUnits.forEach((unit) => {
        ...
        let status = unit.duty == 1 ? "10-41" : '10-42';
    ...
    /* } else if (eventData.type == "bulletin") {
      ...
      $.each(eventData.data, function (index, value) {
        $('.warrants-items').prepend(`<div class="warrants-item" data-cid=${value.cid} data-id=${value.linkedincident}><div style="display: flex; flex-direction: column; margin-top: 2.5px; margin-left: 0.75vh; width: 100%;">
            ...
            <div class="warrant-bottom-info">
                <div class="warrant-id">Sündmuse ID: ${value.linkedincident}</div>
            </div>
        </div></div>`)
    })
      ...
      if (value && IsValidJobForDispatch(value.job, eventData.data.playerJob)) {
        ...
        if (value["automaticGunfire"] == true) {
          DispatchItem += `<div class="call-bottom-info"><span class="fab fa-blackberry"></span>Automaatrelva Lasud</div>`;
        }
        ...
        $(".incidents-items").append(
          `<div class="incidents-item" data-id="${value.id}">
                    <div class="incidents-top-holder">
                        ...
                        <div class="incedent-report-name">Sündmuse Raport</div>
                    </div>
        ...
        $.each(value, function (i, v) {
          $(`#penal-${index}`).append(`
                    ...
                    <div style="display: flex; flex-direction: row; width: 100%; margin: auto; margin-bottom: 0vh; padding-top: 0.75vh;">
                        ...
                        <div class="offfense-item-months">${v.months} Kuud - $${v.fine}</div>
                    </div>
                    `);
        });
      });
      ...
      $(".manage-incidents-editing-title").html(
        "Haldad hetkel sündmust " + table["id"]
      );
        ...
        if (value.associated == 1) {
          $(".incidents-ghost-holder").prepend(
            `<div class="associated-incidents-user-container" data-id="${value.cid}">
                            ...
                            <div class="associated-incidents-user-tags-holder">
                                <div class="associated-incidents-user-tag ${warrantTag}" data-id="${value.cid}">Tagaotsitav</div>
                                <div class="associated-incidents-user-tag ${guiltyTag}" data-id="${value.cid}">Süüdi</div>
                                <div class="associated-incidents-user-tag ${processedTag}" data-id="${value.cid}">Protsessitud</div>
                                <div class="associated-incidents-user-tag ${associatedTag}" data-id="${value.cid}">Seotud</div>
                            </div>
                            <div class="associated-incidents-user-holder" data-name="${value.cid}" style="display:none;">
                            </div>
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">Soovitatav Trahv</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">Soovitatav Karistus</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">Trahv</div>
                            <div class="associated-incidents-fine-input" data-id="${value.cid}" style="display:none;"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="Sisesta trahvisumma..." value="0" class="fine-amount" data-id="${value.cid}" type="number"></div>
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">Karistus</div>
                            <div class="associated-incidents-sentence-input" data-id="${value.cid}" style="display:none;"><img src="https://i.imgur.com/9Xn6xXK.png"> <input placeholder="Sisesta kuude arv..." value="0" class="sentence-amount" data-id="${value.cid}" type="number"></div>
                        </div>`
          );
        } else {
          $(".incidents-ghost-holder").prepend(
            `<div class="associated-incidents-user-container" data-id="${value.cid}">
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">Soovitatav Trahv</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">Soovitatav Karistus</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">Trahv</div>
                            <div class="associated-incidents-fine-input" data-id="${value.cid}"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="Sisesta trahvisumma..." value="0" class="fine-amount" data-id="${value.cid}" type="number"></div>
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">Karistus</div>
                            <div class="associated-incidents-sentence-input" data-id="${value.cid}"><img src="https://i.imgur.com/9Xn6xXK.png"> <input placeholder="Sisesta kuude arv..." value="0" class="sentence-amount" data-id="${value.cid}" type="number"></div>
                        </div>`
          );
        }
          ...
          `
                    <div class="icidents-person-search-item" data-info="${name} (#${value.id})" data-cid="${value.id}" data-name="${name}">
                        ...
                        <div class="icidents-person-search-item-right">
                            <div class="icidents-person-search-item-right-cid-title">Isikukood</div>
                            <div class="icidents-person-search-item-right-cid-input"><span class="fas fa-id-card"></span> ${value.id}</div>
                            <div class="icidents-person-search-item-right-name-title">Nimi</div>
      ...
      let table = eventData.data;
      $(".manage-bolos-editing-title").html(
        "Haldad hetkel BOLO " + table["id"]
      );

      if ($(".badge-logo").attr("src") == "img/ems_badge.png") {
        $(".manage-bolos-editing-title").html(
          "Haldad hetkel intensiivravi Check-in'i " + table["id"]
        );
      }
      ...
      let table = eventData.data;
      var reportName = "Tavaline BOLO";
      canSearchForProfiles = true;
      $(".bolos-items").empty();
      if ($(".badge-logo").attr("src") == "img/ems_badge.png") {
        reportName = "Intensiivravi Check-in";
      }
      $.each(table, function (index, value) {
        $(".bolos-items").prepend(
      ...
      $(".manage-reports-editing-title").html(
        "Haldad hetkel raportit " + table["id"]
      );
      ...
      $(".vehicle-tags").append(
        `<div class="vehicle-tag ${codefive} code5-tag">Kood 5</div>`
      );
      $(".vehicle-tags").append(
        `<div class="vehicle-tag ${stolen} stolen-tag">Varastatud</div>`
      );
      $(".vehicle-info-imageurl-input").val(table["image"]);
      ...
      const impoundDate = localDate.toLocaleDateString("et-EE", {
        timeZone: "UTC",
      });
      const impoundTime = localDate.toLocaleTimeString("et-EE", {
        timeZone: "UTC",
      });
    ...
    $(".profile-items").html(
      `
                      <div class="profile-item" data-id="0">
                          ...
                          <div style="display: flex; flex-direction: column; margin-top: 2.5px; margin-left: 5px; width: 100%; padding: 5px;">
                          <div style="display: flex; flex-direction: column;">
                              <div class="profile-item-title">Ei leitud sellist isikut</div>
                              </div>
                              <div class="profile-bottom-info">
                              </div>
                          </div>
