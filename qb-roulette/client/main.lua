...
createRulettAsztal = function(index, data)
    ...
    local pedOffset = GetObjectOffsetFromCoords(data.position.x, data.position.y, data.position.z - 1, data.rot, 0.0, 0.7, 1.0)
    ...
    SetPedCanEvasiveDive(self.ped, false)
                                        ...
                                        if currentBetAmount > 0 then
                                            if Config.RulettTables[selectedRulett] ~= nil then
                                                if currentBetAmount >= Config.RulettTables[selectedRulett].minBet and currentBetAmount <= Config.RulettTables[selectedRulett].maxBet then
                                                    ...
                                                    QBCore.Functions.Notify('Su panus on liiga suur või väike selle laua jaoks', 'error')
                                                end
                                            end
                                        else
                                            -- Utils.ShowNotification(_U('invalid_bet'))
                                            QBCore.Functions.Notify('Vale panuse summa', 'error')
                                        end
                                    end
                                end
                            end
