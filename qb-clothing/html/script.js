...
QBClothing.ReloadOutfits = function(outfits) {
    ...
    $.each(outfits, function(index, outfit){
        var elem = '<div class="clothing-menu-option" data-myOutfit="'+(index + 1)+'"> <div class="clothing-menu-option-header"><p>'+outfit.outfitname+'</p></div><div class="clothing-menu-myOutfit-option-button"><p>Vali</p></div><div class="clothing-menu-myOutfit-option-button-remove"><p>Kustuta</p></div></div>'
...
QBClothing.Open = function(data) {
    ...
    $(".clothing-menu-changeped-container").css("display", "none");
    ...
    $.each(data.menus, function(i, menu){
        ...
        if (menu.menu == "roomOutfits") {
            $.each(menu.outfits, function(index, outfit){
                var elem = '<div class="clothing-menu-option" data-outfit="'+(index + 1)+'"> <div class="clothing-menu-option-header"><p>'+outfit.outfitLabel+'</p></div> <div class="clothing-menu-outfit-option-button"><p>Vali Outfit</p></div> </div>'
...
QBClothing.SetMaxValues = function(maxValues) {
    $.each(maxValues, function(i, cat){
        if (cat.type == "character") {
            ...
            $(itemMax).html("<p>Mudel: " + maxValues[containers.data('type')].item + "</p>")
            $(headerMax).html("<p>Tekstuur: " + maxValues[containers.data('type')].texture + "</p>")
            ...
            $(itemMax).html("<p>Mudel: " + maxValues[containers.data('type')].item + "</p>")
            $(headerMax).html("<p>Tekstuur: " + maxValues[containers.data('type')].texture + "</p>")
        } else if (cat.type == "changeped") {
            var containers = $(".clothing-menu-changeped-container").find('[data-type="'+i+'"]');
            var itemMax = $(containers).find('[data-headertype="item-header"]');
            var headerMax = $(containers).find('[data-headertype="texture-header"]');
