        ...
        if isLoggedIn and not keyPressed and GetLastInputMethod(0) then
            ...
            if next(keyMeta) ~= nil then
                if keyMeta[v[2]]["command"] ~= "" then
                    ...
                    QBCore.Functions.Notify('Sul ei ole midagi ['..v[2]..'] alla binditud, /binds et bindida command', 'primary', 4000)
                end
            else
                QBCore.Functions.Notify('Sul ei ole ühtegi commandi binditud, /binds et bindida command', 'primary', 4000)
            end
        end
    end, false)
