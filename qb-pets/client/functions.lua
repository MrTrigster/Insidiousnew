...
function getIntoCar()
    ...
    if IsPedSittingInAnyVehicle(plyped) then
        if distance < 8 then
            ...
            ::here::
            if seatEmpty == 6 then
                TriggerEvent('QBCore:Notify', "Pole vaba istekohta")
            end
        else
            TriggerEvent('QBCore:Notify', "Liiga kaugel")
        end
    else
        TriggerEvent('QBCore:Notify', "Pead autos olema")
    end
end
    ...
    while activeLaser do
        ...
        if IsControlJustReleased(0, 38) then
            ...
            if IsPedAPlayer(entity) == 1 or IsEntityAPed(entity) == false or entity == dragger then
                QBCore.Functions.Notify('Ei saa seda teha', "error", 1500)
                return
            end
            CreateThread(function()
                local finished = false
