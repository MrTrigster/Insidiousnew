                ...
                if dealerDist <= 1.5 and not isHealingPerson then
                    if not interacting then
                        if not dealerIsHome then
                            DrawText3D(dealer["coords"]["x"], dealer["coords"]["y"], dealer["coords"]["z"], '[E] Koputa')
                        ...
                        elseif dealerIsHome then
                            if dealer["name"] == "Mystery man" then
                                DrawText3D(dealer["coords"]["x"], dealer["coords"]["y"], dealer["coords"]["z"], '[E] Et osta / [G] Aita oma semu ($5000)')
                            else
                                DrawText3D(dealer["coords"]["x"], dealer["coords"]["y"], dealer["coords"]["z"], '[E] Et osta / [G] Alusta missiooni')
                            end
                            if IsControlJustPressed(0, 38) then
                                buyDealerStuff()
                            end
                            ...
                            if IsControlJustPressed(0, 47) then
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if player ~= -1 and distance < 5.0 then
                                        ...
                                        QBCore.Functions.Progressbar("hospital_revive", "Aitab semu püsti...", 5000, false, true, {
                                            ...
                                            QBCore.Functions.Notify("Sa aitasid isiku püsti")
                                        ...
                                        QBCore.Functions.Notify("Kedagi pole läheduses", "error")
                                    end
                                else
                                    if waitingDelivery == nil then
                                        TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "Siin on kraam, võtan ühendust emaili kaudu")
                                        requestDelivery()
                                        interacting = false
                                        dealerIsHome = false
                                    else
                                        TriggerEvent("chatMessage", "Dealer "..Config.Dealers[currentDealer]["name"], "error", 'Sul on vaja kaup kohale toimetada, mida sa siin teed?!')
                                    end
                                end
                            end
                        end
...
function knockDoorAnim(home)
    ...
    if home then
        ...
        if Config.Dealers[currentDealer]["name"] == "Mystery man" then
            ...
            QBCore.Functions.Notify('Mida ma sinu jaoks teha saan?', 3500)    
        elseif Config.Dealers[currentDealer]["name"] == "Fred" then
            ...
            QBCore.Functions.Notify('Yo '..myData.charinfo.firstname..', mida ma sinu jaoks teha saan?', 3500)    
        end
        -- knockTimeout()
    else
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "knock_door", 0.2)
        ...
        knockingDoor = false
        Wait(1000)
        QBCore.Functions.Notify('Tundub et kedagi ei ole siin', 'error', 3500)
    end
end
...
AddEventHandler('qb-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        ...
        QBCore.Functions.Notify('Sul on ikka kaubavedu pooleli')
        return
    end
        ...
        while true do
            ...
            if activeDelivery ~= nil then
                ...
                if dist < 15 then
                    ...
                    if dist < 1.5 then
                        DrawText3D(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"], activeDelivery["coords"]["z"], '[E] '..activeDelivery["amount"]..'x '..QBCore.Shared.Items[activeDelivery["itemData"]["item"]]["label"]..' Toimeta kohale')
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        checkPedDistance()
        QBCore.Functions.Progressbar("work_dropbox", "Annab kaupa üle...", 3500, false, true, {
...
AddEventHandler('qb-drugs:client:robberyCall', function(msg, streetLabel, coords)
    ...
    AddTextComponentString("911: Narko Tehing")
