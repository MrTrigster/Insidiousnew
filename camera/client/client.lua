            ...
            if IsControlJustPressed(0, 35) then
                ...
                if ActualCamera < NumberCameras then
                    if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(CameraProp[ActualCamera + 1])) < Config.DistanceToConnect then
                        ...
                        QBCore.Functions.Notify('Sa oled liiga kaugel, et kaameraga ühendust luua', 'error')
                    end
                else
                    QBCore.Functions.Notify("Sul ei ole rohkem ühtegi kaamerat läheduses", 'error')
                end
...
RegisterNetEvent("aj-camera:client:PlaceCamera", function()
        ...
        QBCore.Functions.Notify('Vajuta [E] et paigaldada kaamera', 'primary', 7500)
        QBCore.Functions.Notify('Vajuta vasakut ja paremat noolt, et kaamerat liigutada', 'primary', 7500)
                ...
                if CoordsCamera ~= NoLocation then
                    ...
                    if GetEntityCoords(CameraProp[CountCamera]) == NoLocation then
                        if IsControlJustPressed(0, 38) then
                            QBCore.Functions.Notify("Sa ei saa seda siia panna", 'error')
                        end
                    else
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            QBCore.Functions.Notify('Sa paigaldasid kaamera #'..CountCamera, 'success')
                            break
                        end
...
RegisterCommand("cameras", function(source, args, rawCommand)
    if not IsPedInAnyVehicle(PlayerPedId(), true) then
        ...
        if json.encode(ListCameras) == "[]" then
            QBCore.Functions.Notify('Sul ei ole ühtegi kaamerat paigaldatud', 'error')
        else
            if not PlacingCamera then
                if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(CameraProp[1])) < Config.DistanceToConnect then
                    GoPanelCamera()
                else
                    QBCore.Functions.Notify('Sa ei saa kaameraga ühendust', 'error')
                end
            end
        end
    end
