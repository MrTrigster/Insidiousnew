            ...
            if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
                if GetPedInVehicleSeat(veh, -1) == ped then
                    if GetEntitySpeed(veh) <= 1 then
                        QBCore.Functions.Progressbar("use_nos", "Ühendab Nitro...", 45000, false, true, {
                        ...
                        QBCore.Functions.Notify('Sa ei saa liikuvale autole NOS\'i lisada', 'error')
                    end   
                else
                    QBCore.Functions.Notify("Sa ei saa teha seda sellelt istmelt", "error")
                end
            else
                QBCore.Functions.Notify('Sa ei ole sõidukis', 'error')
            end
        else
            QBCore.Functions.Notify('Sa oled Nitro juba paigaldanud', 'error')
        end
    --else
                ...
                if VehicleNitrous[Plate].hasnitro then
                    if NitroMode then
                        if IsControlJustPressed(0, 36) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
                            SetVehicleEnginePowerMultiplier(CurrentVehicle, (NitrousBoost / 1.75) * nitroflowrate)
                            SetVehicleEngineTorqueMultiplier(CurrentVehicle, (NitrousBoost / 1.75) * nitroflowrate)
                            ...
                            CreateThread(function()
                                while NitrousActivated do
                                    if VehicleNitrous[Plate].level - 1 ~= 0 then
                                        ...
                                        TriggerServerEvent('nitrous:server:UpdateNitroLevel', Plate, (VehicleNitrous[Plate].level - nitroflowrate))
                        ...
                        elseif IsControlJustPressed(0, 121) and nitroflowrate <= 2 then
                            ...
                            QBCore.Functions.Notify('NOS Flowrate: ' .. nitroflowrate)
                        elseif IsControlJustPressed(0, 121) and nitroflowrate >= 2 then
                            QBCore.Functions.Notify('NOS on maksimum Flowrate peal')
                        elseif IsControlJustPressed(0, 214) and nitroflowrate >= 2 then
                            nitroflowrate = nitroflowrate - 1
                            QBCore.Functions.Notify('NOS Flowrate: ' .. nitroflowrate)
                        elseif IsControlJustPressed(0, 214) and nitroflowrate <= 2 then
                            QBCore.Functions.Notify('NOS on miinimum Flowrate peal')
                        end
                    elseif PurgeMode then
                        ...
                        elseif IsControlJustPressed(0, 121) and purgeflowrate >= 0.9 then
                            QBCore.Functions.Notify('Purge Spray on maksimum Flowrate peal')
                        elseif IsControlJustPressed(0, 214) and purgeflowrate >= 0.2 then
                            purgeflowrate = purgeflowrate - 0.1
                            QBCore.Functions.Notify('Purge Spray Flowrate: ' .. purgeflowrate)
                        elseif IsControlJustPressed(0, 214) and purgeflowrate <= 0.2 then
                            QBCore.Functions.Notify('Purge Spray on miinimum Flowrate peal')
                        else
                            SetVehicleNitroPurgeEnabled(CurrentVehicle, false)
                    ...
                    if IsControlJustPressed(0, 21) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
                        if not PurgeMode and NitroMode then
                            PurgeMode = true
                            NitroMode = false
                            QBCore.Functions.Notify('Purge Mode')
...
RegisterNetEvent('nitrous:client:UnloadNitrous', function(Plate)
    ...
    TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items['bottlenitrous'], "add")
    TriggerServerEvent("QBCore:Server:AddItem", 'bottlenitrous', 1)
    local CurrentVehicle = GetVehiclePedIsIn(PlayerPedId())
    local CPlate = trim(GetVehicleNumberPlateText(CurrentVehicle))
    if CPlate == Plate then
        NitrousActivated = false
