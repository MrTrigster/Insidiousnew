...
$(document).on('click', '.racing-name-select', function(e){
    e.preventDefault();
    $.post('https://qb-phone/GetCurrentRacerName', JSON.stringify({}), function(data){
        var racer = data.CurrentRacerName;
        $('#racing-name-currentname').html(racer);
    });
    $(".racing-name-modal").fadeIn(200);
});

$(document).on('click', '#racing-name-accept', function(e){
    e.preventDefault();

    var racingName = $(".racing-name-nameinput").val();

    if (racingName !== "" && racingName !== undefined && racingName !== null) {
        racingName = DOMPurify.sanitize(racingName, {
            ALLOWED_TAGS: [],
            ALLOWED_ATTR: []
        });
        if (racingName == '') racingName = 'Tundmatu'
        $.post('https://qb-phone/IsRacerNameAvailable', JSON.stringify({ racerName: racingName }), function(data){
            if (data.IsRNameAvailable) {
                $.post('https://qb-phone/ChangeRacerName', JSON.stringify({
                    RacingName: racingName
                }));
                $(".racing-name-modal").fadeOut(200, function(){
                    $(".racing-name-nameinput").val("");
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "See nimi ei ole saadaval..", "#1DA1F2");
            }
        })
    }
});

$(document).on('click', '#racing-name-cancel', function(e){
    e.preventDefault();
    $(".racing-name-modal").fadeOut(200, function(){
        $(".racing-name-nameinput").val("");
    });
...
$(document).on('click', '.racing-race', function(e){
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Race juba algas", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            var Locked = '<i class="fas fa-unlock"></i> Pole alustatud veel';
            if (race.RaceData.Started) {
                Locked = '<i class="fas fa-lock"></i> Algas';
            }
            var LapLabel = "";
            if (race.Laps == 0) {
                LapLabel = "SPRINT"
                ...
                if (race.Laps == 1) {
                    LapLabel = race.Laps + " Ring";
                } else {
                    LapLabel = race.Laps + " Ringi";
                }
            }
                ...
                if (InDistance) {
                    $.post('https://qb-phone/IsBusyCheck', JSON.stringify({
                        ...
                        if (!IsBusy) {
                            ...
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Sa oled editoris!", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Sa oled juba Race\'is!", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('https://qb-phone/GetTrackData', JSON.stringify({
        ...
        // }
        if ((TrackData.CreatorData.metadata.racername).length > 8) {
            TrackData.CreatorData.metadata.racername = TrackData.CreatorData.metadata.racername.substring(0, 8);
        }
        // var CreatorTag = TrackData.CreatorData.charinfo.firstname.charAt(0).toUpperCase() + ". " + TrackData.CreatorData.charinfo.lastname;
        var CreatorTag = TrackData.CreatorData.metadata.racername;

        $(".racing-setup-information-distance").html('Kaugus: '+TrackData.Distance+' m');
        $(".racing-setup-information-creator").html('Looja: ' + CreatorTag);
        if (TrackData.Records.Holder !== undefined) {
            ...
            // }
            if (TrackData.Records.Holder[0].length > 8) {
                TrackData.Records.Holder[0] = TrackData.Records.Holder[0].substring(0, 8) + "..";
            }
            // var Holder = TrackData.Records.Holder[0].charAt(0).toUpperCase() + ". " + TrackData.Records.Holder[1];
            var Holder = TrackData.Records.Holder[0];
            ...
            if (!data.IsBusy) {
                $.post('https://qb-phone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Sa oled Race\'is..", "#1DA1F2");
                    }
                });
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Sa juba teed rada..", "#1DA1F2");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Sul ei ole luba raja tegemiseks..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        ...
        if (TrackName == '') TrackName = 'Mida sa üritad?'
        $.post('https://qb-phone/IsAuthorizedToCreateRaces', JSON.stringify({ TrackName: TrackName }), function(data){
            if (data.IsAuthorized) {
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Pead sisestama nime..", "#1DA1F2");
    }
});
                            ...
                            $.post('https://qb-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    ...
                                    $(".racing-setup").animate({
                                        ...
                                        $(".racing-setup-information-distance").html('Vali Rada');
                                        $(".racing-setup-information-creator").html('Vali Rada');
                                        $(".racing-setup-information-wr").html('Vali Rada');
                                        $(".racing-setup-laps").val("");
                                        $('.dropdown').find('input').removeAttr('value');
                                        $('.dropdown').find('span').text("Vali Rada");
                                    });
                                } else {
                                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Sa ei saa seda teha ..", "#1DA1F2");
                                }
                            });
                        } else {
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Sisesta ringide arv..", "#1DA1F2");
                        }
                    } else {
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Sa ei ole rada valinud..", "#1DA1F2");
                    }
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Sul juba aktiivne sõit..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '.racing-leaderboard-item', function(e){
    ...
    $.each(Data.LastLeaderboard, function(i, leaderboard){
        ...
        var elem = '<div class="row"> <div class="name">' + name + '</div><div class="time">'+bestroundtime+'</div><div class="score">'+ place +'</div> </div>';
        ...
        if (Races !== null) {
            ...
            $.each(Races, function(i, race){
                if (race.LastLeaderboard.length > 0) {
                    var elem = '<div class="racing-leaderboard-item" id="leaderboard-item-'+i+'"> <span class="racing-leaderboard-item-name"><i class="fas fa-flag-checkered"></i> '+race.RaceName+'</span> <span class="racing-leaderboard-item-info">Vajuta et näha rohkem detaile</span> </div>'
