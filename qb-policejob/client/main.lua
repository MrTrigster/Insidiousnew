    ...
    SetTimeout(math.random(2500, 4000), function()
        ...
        TriggerServerEvent('i13-phone:server:sendNewMail', {
            ...
            message = "Tervist " .. gender .. " " .. charinfo.lastname ..
                ",<br /><br />Maksuamet võttis Teie kontolt politsei poolt tehtud arve summa.<br />Teie kontolt võeti <strong>$" ..
                amount .. "</strong> suurune summa.<br /><br />Kõike head,<br />R. Kalashnikov",
            button = {}
        })
    end)
end)
...
RegisterNetEvent('police:client:policeAlert', function(coords, text)
    ...
    local blipText = 'Politsei Alert - ' ..text
...
RegisterNetEvent('police:client:policeEmergencyAlert', function(coords)
    local street1, street2 = GetStreetNameAtCoord(coords.x, coords.y, coords.z, Citizen.ResultAsInteger(), Citizen.ResultAsInteger())
    local street1name = GetStreetNameFromHashKey(street1)
    local street2name = GetStreetNameFromHashKey(street2)
    QBCore.Functions.Notify({text = 'Häirenupp', caption = street1name.. ' ' ..street2name}, 'police')
    PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
    local transG = 250
    local blip = AddBlipForCoord(coords.x, coords.y, coords.z)
    local blip2 = AddBlipForCoord(coords.x, coords.y, coords.z)
    local blipText = 'Häirenupp'
    SetBlipSprite(blip, 60)
    SetBlipSprite(blip2, 161)
    SetBlipColour(blip, 1)
    SetBlipColour(blip2, 1)
