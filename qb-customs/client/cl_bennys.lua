...
--[[CreateThread(function()
    for k, v in pairs(bennyGarages) do
        if v.blip then
            ...
            AddTextComponentString("Benny's Customs")
...
RegisterNetEvent('event:control:bennys', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        bennyLocation = bennyGarages[useID].coords
        bennyHeading = bennyGarages[useID].heading
            ...
            for k, v in pairs(bennyGarages) do
                ...
                if nearDefault then
                    if not isPlyInBennys then
                        Draw3DText(v.coords.x, v.coords.y, v.coords.z + 0.5, "[~p~E~w~ - Tuuni Autot]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
