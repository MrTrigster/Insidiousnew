...
local function ItemsToItemInfo()
	itemInfos = {
		[1] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 5x, " ..QBCore.Shared.Items["steel"]["label"] .. ": 7x, " ..QBCore.Shared.Items["plastic"]["label"] .. ": 5x, " ..QBCore.Shared.Items["aluminum"]["label"] .. ": 10x"},
		[2] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 4x, " ..QBCore.Shared.Items["steel"]["label"] .. ": 8x, " ..QBCore.Shared.Items["aluminum"]["label"] .. ": 5x"},
        [3] = {costs = QBCore.Shared.Items["steel"]["label"] .. ": 30x, " ..QBCore.Shared.Items["plastic"]["label"] .. ": 50x"},
        [4] = {costs = QBCore.Shared.Items["pistol_ammo"]["label"] .. ": 5x, " ..QBCore.Shared.Items["rifle_ammo"]["label"] .. ": 5x"},
        [5] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 5x, " ..QBCore.Shared.Items["plastic"]["label"] .. ": 10x, " ..QBCore.Shared.Items["gunpowder"]["label"] .. ": 10x"},
	}
...
-- 	itemInfos = {
-- 		[1] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 10x, " .. QBCore.Shared.Items["steel"]["label"] .. ": 12x, " .. QBCore.Shared.Items["rubber"]["label"] .. ": 3x"},
-- 		[2] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 16x, " .. QBCore.Shared.Items["steel"]["label"] .. ": 18x, " .. QBCore.Shared.Items["rubber"]["label"] .. ": 4x"},
-- 		[3] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 18x, " .. QBCore.Shared.Items["steel"]["label"] .. ": 20x, " .. QBCore.Shared.Items["rubber"]["label"] .. ": 8x"},
-- 		[4] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 20x, " .. QBCore.Shared.Items["steel"]["label"] .. ": 22x, " .. QBCore.Shared.Items["rubber"]["label"] .. ": 8x, " .. QBCore.Shared.Items["smg_extendedclip"]["label"] .. ": 1x"},
-- 	}
...
local function GetThresholdItems()
	...
	local items = {}
	for k, item in pairs(Config.CraftingItems["items"]) do
		if PlayerData.metadata["craftingrep"] >= Config.CraftingItems["items"][k].threshold then
...
RegisterNetEvent('inventory:client:CraftItems', function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    QBCore.Functions.Progressbar("repair_vehicle", "Craftib...", (math.random(2000, 5000) * amount), false, true, {
...
RegisterNetEvent('inventory:client:PickupSnowballs', function()
    ...
    QBCore.Functions.Progressbar("pickupsnowball", "Korjab lumepalli...", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('inventory:client:openCrafting', function()
    local crafting = {}
                ...
                if vehicle ~= 0 and vehicle ~= nil then
                    ...
                    if #(pos - trunkpos) < 2.0 and not IsPedInAnyVehicle(ped) then
                        if GetVehicleDoorLockStatus(vehicle) < 2 then
                            ...
                            QBCore.Functions.Notify("Sõiduk on lukus", "error")
                            return
                        end
                    else
                        CurrentVehicle = nil
...
RegisterNetEvent('i13-inventory:client:giveAnim', function()
...
RegisterNUICallback("GiveItem", function(data)
    ...
    if player ~= -1 and distance < 3 then
        if (data.inventory == 'player') then
            ...
            QBCore.Functions.Notify("Sul ei ole seda eset", "error")
        end
    else
...
CreateThread(function()
    exports['qb-target']:AddBoxZone('itemcrafting', vector3(97.58, 6618.98, 32.47), 0.7, 1.8, {
        name = 'itemcrafting',
        heading = 315.0,
        debugPoly = false,
        minZ = 31.47,
...
-- 		local pos = GetEntityCoords(PlayerPedId())
-- 		local inRange = false
-- 		local distance = #(pos - vector3(Config.CraftingItems['location'].x, Config.CraftingItems['location'].y, Config.CraftingItems['location'].z))
...
CreateThread(function()
    ...
    while true do
        ...
        for k,v in pairs(Config.GangStashes) do
            local distance = #(pos - v.location)

            if distance < 1.5 and QBCore.Functions.GetPlayerData().gang.name == v.gang then
                -- DrawText3D(v.location.x, v.location.y, v.location.z, "~g~E~w~ - Kapp")
                inRange = true
                text = '[E] - Stash'
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "gang_"..k, {
                        maxweight = 2000000,
                        slots = 300,
                    })
                    TriggerEvent("inventory:client:SetCurrentStash", "gang_"..k)
                end
            end
        end
...
CreateThread(function()
    ...
    while true do
        ...
        for k,v in pairs(Config.JobStashes) do
            local distance = #(pos - v.location)

            if distance < 1.5 and QBCore.Functions.GetPlayerData().job.name == v.job then
                -- DrawText3D(v.location.x, v.location.y, v.location.z, "~g~E~w~ - Kapp")
                inRange = true
                text = '[E] - Stash'
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "job_"..k, {
                        maxweight = 2000000,
                        slots = 300,
                    })
                    TriggerEvent("inventory:client:SetCurrentStash", "job_"..k)
                end
            end
        end
...
CreateThread(function()
    ...
    while true do
        ...
        for k,v in pairs(Config.PersonalStashes) do
            local distance = #(pos - v.location)

            if distance < 1.5 and QBCore.Functions.GetPlayerData().citizenid == v.cid then
                -- DrawText3D(v.location.x, v.location.y, v.location.z, "~g~E~w~ - Kapp")
                inRange = true
                text = '[E] - Stash'
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "personal_"..k, {
                        maxweight = 2000000,
                        slots = 300,
                    })
                    TriggerEvent("inventory:client:SetCurrentStash", "personal_"..k)
                end
            end
        end
