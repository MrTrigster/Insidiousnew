...
function ResetModals(type, name)
{
...
var lang = [];
lang["personal"] = "Isiklik Konto";
lang["business"] = "Firma Konto";
lang["organization"] = "Organisatsiooni Konto";
lang["deposit"] = "Sissemakse";
lang["withdraw"] = "Väljamakse";
lang["transfer"] = "Ülekanne";

var relang = [];
relang["Isiklik Konto"] = "personal";
relang["Firma Konto"] = "business";
relang["Organisatsiooni Konto"] = "organization";
...
\
                        ...
                        <div class='d-flex justify-content-end mt-1 mb-1'>\
                            <p class='card-subtitle'>Saadaval Vahendid</p>\
                            \
                        </div>\
                    </div>\
                </div>\
\
                ...
                <div class='d-flex justify-content-between'>\
                    <button type='button' class='btn btn-light mb-2 text-muted deposit' data-toggle='modal' data-target='#DepositModal' onClick='ResetModals(\"" + transferAcc + "\", \"" + account_name + "\", \"" + "\")'>SISSEMAKSE</button>\
                    <button type='button' class='btn btn-light mb-2 text-muted withdraw' data-toggle='modal' data-target='#WithdrawModal' onClick='ResetModals(\"" + transferAcc + "\", \"" + account_name + "\", \"" + "\")'>VÄLJAMAKSE</button>\
                    <button type='button' class='btn btn-light mb-2 text-muted transfer' data-toggle='modal' data-target='#TransferModal' onClick='ResetModals(\"" + transferAcc + "\", \"" + account_name + "\", \"" + "\")'>ÜLEKANNE</button>\
                </div>\
            </div>\
    </div><br/></div>");
...
function timeSince(date) {
    ...
    if (interval > 1) {
      return Math.floor(interval) + " aastat";
    }
    interval = seconds / 2592000;
    if (interval > 1) {
      return Math.floor(interval) + " kuud";
    }
    interval = seconds / 86400;
    if (interval > 1) {
      return Math.floor(interval) + " päeva";
    }
    interval = seconds / 3600;
    if (interval > 1) {
      return Math.floor(interval) + " tundi";
    }
    interval = seconds / 60;
    if (interval > 1) {
      return Math.floor(interval) + (Math.floor(interval) > 1 && " minutit" ||  " minut");
    }
    return Math.floor(seconds) + " sekundit";
  }
...
function AddTransaction(trans_id, account, amount, time, note, why, receiver, plName)
{
    ...
    {
        $("#" + trans_id + "_time").html(TimeSince + " tagasi")
        return;
    }
            ...
            <div class='card-body'>\
                ...
                <div class='row game-header'>\
                    ...
                    <div class='col d-flex justify-content-end'>\
                        <p class='header-title'><span class='badge bg-primary'>Ülekanne</span> <> <span class='badge bg-secondary'>" + lang[why] + "</span></p>\
                    </div>\
                    <hr/>\
                    <div class='col-2'>\
                    ...
                    <div class='col-5'>\
                        ...
                        <div class='col d-flex justify-content-end'>\
                            <div>\
                                <p class='note' id='"+trans_id+"_time'>" + TimeSince + " tagasi</p>\
...
function OpenATM(data, transactions, name)
{
    if (data && data !== null)
    {
        ...
        {
            let tTbl = tbl[i];
            AddAccount((tTbl.type === "business" && tTbl.name || tTbl.type === "organization" && tTbl.name || "Isiklik Konto"), (lang[tTbl.type] && lang[tTbl.type] || tTbl.type), tTbl.amount, name);
        }
    }
...
Listeners["edit_account"] = function(data)
{
    ...
    if (data) {
        ...
        for (var i = 0; i < auths.length; i++)
        {
            ...
            <td>" + v.target_name + "</td>\
            <td>" + TimeSince + "(s) tagasi</td>\
            <td><button class='btn btn-danger btn-sm' onclick='confirmRemove(\"" + v.target_identifier + "\", \"" + v.target_name + "\")'>Eemalda</button></td>\
            ");
        }
        $("#editAccountModal").modal('toggle');
    } else {
...
Listeners["refresh_balances"] = function(data) {
    ...
    for (var i = 0; i < tbl.length; i++)
    {
        ...
        let account_name = (tTbl.type === "business" && tTbl.name || tTbl.type === "organization" && tTbl.name || "Isiklik Konto");
        let temp_name = account_name.replace(/"|'/g,'');
        let temp_Name = account_name.replace(/\s+/g, '');
