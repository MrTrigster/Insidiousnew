...
--             QBCore.Functions.Notify("Cruise Aktiveeritud: " .. CruisedSpeedKm ..  " KM/H") -- Uncomment me for km/h
...
--                         CruisedSpeed = 0
--                         TriggerEvent('seatbelt:client:ToggleCruise')
--                         QBCore.Functions.Notify("Cruise Väljas", "error")
--                         Wait(2000)
--                         break
--                     end
...
local cruiseActivated = false
local function TriggerCruiseControl()
    CruisedSpeed = GetVehicleSpeed()
    CruisedSpeedKm = TransformToKm(CruisedSpeed)
    SetEntityMaxSpeed(GetVehicle(), GetVehicleSpeed())
    QBCore.Functions.Notify("Piiraja Aktiivne: " .. CruisedSpeedKm .. " km/h") -- Uncomment me for km/h
    TriggerEvent('seatbelt:client:ToggleCruise')
...
RegisterCommand('togglecruise', function()
    ...
    if IsDriver() then
        if vehicleClasses[vehClass] then
            ...
            QBCore.Functions.Notify("Cruise ei saa sellel kasutada", "error")
        end
    end
end, false)
