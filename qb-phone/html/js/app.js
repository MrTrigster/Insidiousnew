...
QB.Phone.Functions.SetupApplications = function(data) {
    QB.Phone.Data.Applications = data.AppData;
    var dongle = data.raceDongle;
    ...
    $.each(data.AppData, function(i, app){
...
$(document).on('click', '.phone-application', function(e){
    ...
    if (AppObject.length !== 0) {
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $("#mySerialNumber").text("I13-" + QB.Phone.Data.PlayerData.metadata["phonedata"].SerialNumber);
                ...
                } else if (PressedApplication == "fines") {
                    $.post('https://qb-phone/GetFines', JSON.stringify({}), function(fines){
                        QB.Phone.Functions.LoadFines(fines);
                    });
                } else if (PressedApplication == "jobs") {
                    $.post('https://qb-phone/GetJobs', JSON.stringify({}), function(jobs){
                        QB.Phone.Functions.LoadJobs(jobs);
                    });
                    $.post('https://qb-phone/GetCurrentJob', JSON.stringify({}), function(currentjob){
                        $.post('https://qb-phone/GetCurrentDuty', JSON.stringify({}), function(currentduty){
                            QB.Phone.Functions.LoadCurrentJob(currentjob, currentduty);
                        });
                    });
        ...
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "System", QB.Phone.Data.Applications[PressedApplication].tooltipText+" ei ole saadaval!")
    }
});
...
$(document).on('click', '.phone-home-container', function(event){
    ...
    if (QB.Phone.Data.currentApplication === null) {
        ...
        } else if (QB.Phone.Data.currentApplication == "jobs") {
        ...
        if (HasPhone) {
            ...
            if (QB.Phone.Notifications.Timeout == undefined || QB.Phone.Notifications.Timeout == null) {
                if (color != null || color != undefined) {
                    $(".notification-icon-container").css({"background":color});
                    $(".notification-title").css({"color":"#eeeeee"});
                } else if (color == "default" || color == null || color == undefined) {
                    $(".notification-icon").css({"color":"#eeeeee"});
                    $(".notification-title").css({"color":"#eeeeee"});
                }
                if (!QB.Phone.Data.IsOpen) {
                    QB.Phone.Animations.BottomSlideUp('.container', 300, -56);
                }
                QB.Phone.Animations.TopSlideDown(".phone-notification-container", 200, 5);
                if (icon !== "politie") {
                ...
                if (color != null || color != undefined) {
                    $(".notification-icon-container").css({"background":color});
                    $(".notification-title").css({"color":"#eee"});
                } else {
...
QB.Phone.Notifications.Boosting = function(icon, title, text, color, toggle) {
    $.post('https://qb-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
            if (toggle === true) {
                $(".notification-icon-container").css({"background":color});
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            ...
            case "IncomingCallAlert":
                $.post('https://qb-phone/HasPhone', JSON.stringify({}), function(HasPhone){
                    if (HasPhone) {
                        ...
                        IncomingCallQuake(event.data.CallData, event.data.Canceled, event.data.AnonymousCall);
                    }
                });
                break;
            case "SetupHomeCall":
            ...
            case "UpdateCallTime":
                ...
                //     }
                //     $(".call-notifications-title").html("Kõnes ("+timeString+")");
                //     $(".call-notifications-content").html("Isikuga "+event.data.Name);
                ...
                // $(".phone-currentcall-title").html("Kõnes ("+timeString+")");
                // break;
                var CallTime = event.data.Time;
                var date = new Date(null);
                date.setSeconds(CallTime);
            ...
            case "CancelOngoingCall":
                ...
                if (!QB.Phone.Data.IsOpen) {
                    QB.Phone.Animations.BottomSlideUp('.container', 300, -70);
                }
