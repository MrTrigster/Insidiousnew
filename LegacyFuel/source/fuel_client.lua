	...
	options = {
		{
			...
			icon = "fas fa-burn",
			label = "Osta Petrol Can",
		},
		{
			type = "client",
			event = "lj-fuel:client:refuelCanMenu",
			icon = "fas fa-gas-pump",
			label = "Täida Petrol Can",
		},
	},
		distance = 2.0
})
...
AddEventHandler('lj-fuel:client:buyCanMenu', function()
    exports['qb-menu']:openMenu({
        {
            header = "Tankla",
            txt = 'Kogusumma tuleb: $'..Config.canCost..' maksud kaasaarvatud',
            params = {
...
AddEventHandler('lj-fuel:client:refuelCanMenu', function()
	exports['qb-menu']:openMenu({
		{
			header = "Tankla",
			txt = "Osta Petrol Can. Lisandub 10% maksud.",
			params = {
...
RegisterNetEvent('lj-fuel:client:refuelCan', function()
	...
	if GetAmmoInPedWeapon(ped, 883325847) < 4500 then
		...
		while not HasAnimDictLoaded('weapon@w_sp_jerrycan') do Citizen.Wait(100) end
			...
			QBCore.Functions.Progressbar("refuel-car", "Täidab...", 10000, false, true, {
			...
			QBCore.Functions.Notify('See Petrol Can on juba täis', 'error')
		end
...
RegisterNetEvent('lj-fuel:client:SendMenuToServer', function()
	...
	if CurFuel < 95 then
		...
		QBCore.Functions.Notify('Sõiduki paak on juba täis', 'error')
	end
end)

-- refuel vehicle 
RegisterNetEvent('lj-fuel:client:RefuelVehicle', function(refillCost, tax)
	local vehicle = QBCore.Functions.GetClosestVehicle()
	local ped = PlayerPedId()
	local CurFuel = GetVehicleFuelLevel(vehicle)
	...
	if inGasStation == false and not HasPedGotWeapon(ped, 883325847) then
		QBCore.Functions.Notify('Sul ei ole Petrol Can\'i', 'error')
	elseif inGasStation == false and GetAmmoInPedWeapon(ped, 883325847) == 0 then
		QBCore.Functions.Notify('Petrol Can on tühi', 'error')
		return
	end
	...
	if HasPedGotWeapon(ped, 883325847) then
		...
		while not HasAnimDictLoaded('weapon@w_sp_jerrycan') do 
			...
			QBCore.Functions.Progressbar("refuel-car", "Tangib...", time, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
	...
	if inGasStation then
		if isCloseVeh() then
			...
			QBCore.Functions.Progressbar("refuel-car", "Tangib...", time, false, true, {
				...
				TriggerServerEvent('lj-fuel:server:PayForFuel', refillCost, tax, GetPlayerServerId(PlayerId()))
				SetFuel(vehicle, 100)
				PlaySound(-1, "5_SEC_WARNING", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
				StopAnimTask(ped, "weapon@w_sp_jerrycan", "fire", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
			end, function() -- Cancel
...
function ManageFuelUsage(vehicle)
	...
	if IsVehicleEngineOn(vehicle) then
		SetFuel(vehicle, GetVehicleFuelLevel(vehicle) - Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] * 1.0 / 10) --Config.Classes[GetVehicleClass(vehicle)] or 
		SetVehicleEngineOn(veh, true, true, true)
else
	SetVehicleEngineOn(veh, true, true, true)
