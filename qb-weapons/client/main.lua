    ...
    if CurrentWeaponData then
        if QBCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and QBCore.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            ...
            if total < maxAmmo then
                QBCore.Functions.Progressbar("taking_bullets", "Laeb salve...", math.random(4000, 6000), false, true, {
                    ...
                    if QBCore.Shared.Weapons[weapon] then
                        ...
                        TriggerEvent('QBCore:Notify', 'Relv laetud', "success")
                    end
                end, function()
                    QBCore.Functions.Notify("Tühistatud", "error")
                end)
            else
                QBCore.Functions.Notify("Ei mahu rohkem kuule", "error")
            end
        else
            QBCore.Functions.Notify("Sul ei ole relva", "error")
        end
    else
        QBCore.Functions.Notify("Sul ei ole relva", "error")
    end
...
RegisterNetEvent('weapons:client:ScratchSerialNumber', function(itemData)
    local ped = PlayerPedId()
    local weapon = GetSelectedPedWeapon(ped)
    if CurrentWeaponData then
        if QBCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" then
            QBCore.Functions.Progressbar("taking_bullets", "Kraabib seerianumbrit...", math.random(4000, 6000), false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
                ...
                if QBCore.Shared.Weapons[weapon] then
                    TriggerEvent('QBCore:Notify', 'Seerianumber kraabitud', "success")
                    TriggerServerEvent("weapons:server:ScratchSerialNumber", CurrentWeaponData)
                end
            end, function()
                QBCore.Functions.Notify("Tühistatud", "error")
            end)
        else
            QBCore.Functions.Notify("Sul ei ole vajalikku eset käes", "error")
        end
    else
        QBCore.Functions.Notify("Sul ei ole vajalikku eset", "error")
    end
end)
    ...
    if weapon ~= GetHashKey("WEAPON_UNARMED") then
        ...
        if WeaponAttachments[WeaponData.name] then
            if WeaponAttachments[WeaponData.name][attachment]['item'] == ItemData.name then
                ...
                QBCore.Functions.Notify("Sellele relvale ei sobi see attatchment", "error")
            end
        end
    else
        QBCore.Functions.Notify("Sul ei ole relva käes", "error")
    end
end)
                ...
                if IsPedShooting(ped) or IsControlJustPressed(0, 24) then
                    ...
                    if CanShoot then
                        ...
                        if weapon ~= -1569615261 then
                            ...
                            QBCore.Functions.Notify("See relv on katki ja seda ei saa enam kasutada", "error")
                            MultiplierAmount = 0
                        end
                    end
                end
                    ...
                    if distance < 1 then
                        if data.IsRepairing then
                            if data.RepairingData.CitizenId ~= PlayerData.citizenid then
                                DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, 'Relvaparandust ei saa hetkel kasutada')
                            else
                                if not data.RepairingData.Ready then
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, 'Su relv tehakse korda')
                                else
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, '[E] - Võta relv tagasi')
                                end
                            end
                        else
                            if CurrentWeaponData and next(CurrentWeaponData) then
                                if not data.RepairingData.Ready then
                                    ...
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, '[E] Paranda relva, ~g~$'..Config.WeaponRepairCosts[WeaponClass]..'~w~')
                                ...
                                if data.RepairingData.CitizenId == nil then
                                    DrawText3Ds(data.coords.x, data.coords.y, data.coords.z, 'Sul ei ole relva käes')
