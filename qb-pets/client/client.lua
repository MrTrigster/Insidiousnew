...
function addXpForDistanceMoved(savedData)
    ...
    if distance > 0 and IsPedInAnyVehicle(savedData.entity, true) ~= 1 then
        ...
        if Xp >= currentMaxXP then
            ...
            TriggerEvent('QBCore:Notify', savedData.itemData.info.name .. " Level Up: " .. savedData.level)
        else
            ActivePed:update {
                index = index,
                xp = Xp
...
AddEventHandler('keep-companion:client:callCompanion', function(modelName, hostileTowardPlayer, item)
    ...
    CoreName.Functions.Progressbar("callCompanion", "Kutsub looma...", Config.Settings.callCompanionDuration * 1000,
        false, false, {
...
AddEventHandler('keep-companion:client:increaseHealth', function(ped, item, TYPE)
    QBCore.Functions.TriggerCallback("QBCore:HasItem", function(hasitem)
        if hasitem then
            ...
            -- firstaidforpet
            CoreName.Functions.Progressbar("reviveing", "Elustab...",
            ...
            QBCore.Functions.Notify('Sul on vaja Esmaabi pakki loomadele, et elustada', "error", 1500)
        end
    end, 'firstaidforpet')
end)
...
AddEventHandler('keep-companion:client:despawn', function(ped, item, revive)
    ...
    CoreName.Functions.Progressbar("despawn", "Kutsub tagasi...", Config.Settings.despawnDuration * 1000, false, false, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = false
...
AddEventHandler('keep-companion:client:getPetdata', function()
    CoreName.Functions.Progressbar("feeding", "Toidab...", Config.Settings.feedingSpeed * 1000, false, false, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = false
...
RegisterNetEvent('keep-companion:client:renameCollar', function(item)
    if ActivePed:read() ~= nil then
        ...
        QBCore.Functions.Notify('Loom peab aktiivne olema', 'error', 7500)
    end
...
RegisterNetEvent('keep-companion:client:renameCollarAction', function(name)
    ...
    if activePed ~= nil then
        ...
        if next(currentItem) ~= nil and type(name) == "string" and validation == false then
            CoreName.Functions.Progressbar("waitingForName", "Muudab nime...",
        ...
        elseif validation.reason == 'badword' then
            TriggerEvent('QBCore:Notify', "Sa ei saa seda nime panna")
            print_table(validation.words)
        elseif validation.reason == 'maxCharacter' then
            TriggerEvent('QBCore:Notify', "Liiga pikk nimi")
        elseif validation.reason == 'moreThanOneWord' then
            -- won't trigger
            TriggerEvent('QBCore:Notify', "Nimi peab olema ühesõnaline")
        end
    else
        TriggerEvent('QBCore:Notify', "Loom peab aktiivne olema")
    end
end)
