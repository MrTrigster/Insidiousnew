...
function SetLaststand(bool, spawn)
	...
	--TriggerServerEvent('hospital:server:ambulanceAlert', 'Isik Haavatud')
        ...
        CreateThread(function()
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    QBCore.Functions.Notify("Sa jooksid verest tühjaks", "error")
                    SetLaststand(false)
                    ...
                    TriggerServerEvent("i13-log:server:CreateLog", "death", GetPlayerName(player) .. " ("..GetPlayerServerId(player)..") is dead", "red", "**".. killerName .. "** has killed  ".. GetPlayerName(player) .." with a **".. weaponLabel .. "** (" .. weaponName .. ")")
                    deathTime = 0
                    OnDeath()
                    DeathTimer()
                end
...
RegisterNetEvent('hospital:client:HelpPerson', function(targetId)
    ...
    QBCore.Functions.Progressbar("hospital_revive", "Elustab isikut...", math.random(30000, 60000), false, true, {
        ...
        ClearPedTasks(ped)
        QBCore.Functions.Notify("Sa elustasid isikut")
