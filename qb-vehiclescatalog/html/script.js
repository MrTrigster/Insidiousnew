...
function menuVehicle(event) {
    ...
    $.post('https://qb-vehiclescatalog/menuSelected', JSON.stringify({menuId: categoryVehicleSelected.toLowerCase()}));
}

function testDrive(){
    $.post('https://qb-vehiclescatalog/TestDrive', JSON.stringify({vehicleModel: vehicleSelected.modelcar}));    
...
function buyVehicle(option) {
    ...
    switch(option){
        case 'cancel':
            break;
        case 'confirm':
            $.post('https://qb-vehiclescatalog/Buy', JSON.stringify(vehicleSelected));
            break;
    }   
}
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 27: // ESC
            $.post('https://qb-vehiclescatalog/Close');
            $('body').css("display","none");
        ...
        case 17: // TAB
            break;
        case 68: // LEFT A
            $.post('https://qb-vehiclescatalog/rotate', JSON.stringify({key: "left"}))
            break;            
        case 65: // RIGHT D
            $.post('https://qb-vehiclescatalog/rotate', JSON.stringify({key: "right"}))
            break;
        case 39: 
...
$(document).on('keydown', function(ev) {
    ...
    if (_key) {
        $.post('https://qb-vehiclescatalog/rotate', JSON.stringify({key: _key}))
    }
    ...
    switch(event.which) {        
        ...
        case 1:
            ...
            if(!itemDisabled) {
                ...
                $('#nameBrand').append(`               
                    <span id="vehicle_name">`+dataCar[1].outerText+` | `+dataCar[11].outerText+` Class</span>
                ...
                $.post("https://qb-vehiclescatalog/SpawnVehicle", JSON.stringify({ modelcar: dataCar[9].outerText, price: dataCar[7].outerText }));
                ...
                <div class="item-cars"> 
                    <div class="col-lg-3 col-md-6 "> 
                        <div class="specification" style="opacity:0.0; position:absolute;">
                            ...
                            <span id="class">`+ data[i].class +`</span>
                        </div> 
                        <div class="soldOut">SOLD OUT</div>                 
                        <div class="img-fluid" style="background-image: url(../imgs/` + modelUper.toUpperCase() +`.png);"> <span class="exibeClass">`+ data[i].class +` Class</span> <span class="exibeName">`+ data[i].name +`</span></div>
                    </div>
                </div>`);
            }                 
...
function setVehicleColorRGB(R,G,B){
    ...
    if (selectedColor == 'primary'){
        $.post("https://qb-vehiclescatalog/RGBVehicle", JSON.stringify({primary: true, R: R, G: G, B: B}));
    } else {
        $.post("https://qb-vehiclescatalog/RGBVehicle", JSON.stringify({primary: false, R: R, G: G, B: B}));
    }
}
