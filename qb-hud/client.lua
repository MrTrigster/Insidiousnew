...
local function loadSettings(settings)
    ...
    --QBCore.Functions.Notify('HUD Laetud', "success")
    Wait(1000)
...
local function restartHud()
    ...
    QBCore.Functions.Notify('HUD Restardib', "error")
    ...
    Wait(2600)
    QBCore.Functions.Notify('HUD Restartitud', "success")
end
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        if Menu.isMapNotifChecked then
            QBCore.Functions.Notify('Square Map Laeb')
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            QBCore.Functions.Notify('Square Map Laetud')
        end
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        if Menu.isMapNotifChecked then
            QBCore.Functions.Notify('Circle Map Laeb')
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            QBCore.Functions.Notify('Circle Map Laetud')
        end
    end
end)
...
RegisterCommand('+engine', function()
    ...
    if GetIsVehicleEngineRunning(vehicle) then
        QBCore.Functions.Notify('Mootor Seisatud')
    else
        QBCore.Functions.Notify('Mootor Käivitatud')
    end
    SetVehicleEngineOn(vehicle, not GetIsVehicleEngineRunning(vehicle), false, true)
end)

RegisterKeyMapping('+engine', 'Toggle Engine', 'keyboard', 'J')
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports['LegacyFuel']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        ...
                        QBCore.Functions.Notify('Kütuse tase madal', "error")
...
RegisterNetEvent('hud:client:OnMoneyChange', function(type, amount, isMinus)
    ...
    chipsAmount = PlayerData.money['chips']
    SendNUIMessage({
        action = 'updatemoney',
        cash = cashAmount,
        bank = bankAmount,
