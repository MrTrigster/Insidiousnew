...
RegisterNetEvent('fillTaser', function(source, args, rawCommand)
	if taserCartsLeft <= 0 then
		QBCore.Functions.Progressbar('load_taser', 'Laeb Taserit...', 2000, false, true, {
		...
		QBCore.Functions.Notify('Sul ei ole vaja taserit laadida', 'error')
	end
end)
		...
		if taserCartsLeft <= 0 then
			if GetSelectedPedWeapon(ped) == taserModel then
				...
				if IsControlJustReleased(0, 106) then
					QBCore.Functions.Notify('Pead taserit laadima', 'error')
				end
			end
		end
