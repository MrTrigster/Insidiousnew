...
-- RegisterKeyMapping('mdt', 'Open Police MDT', 'keyboard', 'DELETE')
...
RegisterNetEvent('mdt:client:open', function(bulletin, activeUnits, calls, cid)
    ...
    SendNUIMessage({ type = "data", activeUnits = activeUnits, citizenid = cid, ondutyonly = Config.OnlyShowOnDuty, name = "Tere, " ..PlayerData.job.grade.label..' '..PlayerData.charinfo.lastname:sub(1,1):upper()..PlayerData.charinfo.lastname:sub(2), location = playerStreetsLocation, fullname = PlayerData.charinfo.firstname..' '..PlayerData.charinfo.lastname, bulletin = bulletin })
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS asukoht märgitud!', 'success')
end)
...
RegisterNUICallback("saveVehicleInfo", function(data, cb)
    ...
    if JobType == 'police' and impound.impoundChanged == true then
        if impound.impoundActive then
            ...
            if found == 0 then
                QBCore.Functions.Notify('Sõidukit ei leitud!', 'error')
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        QBCore.Functions.Notify("Ühendasid raadiokanalile "..radio..".", "success")
    else
        QBCore.Functions.Notify("Vale kanal(sisesta number)", "error")
    end
end)
