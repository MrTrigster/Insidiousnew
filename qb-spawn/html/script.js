$(document).ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (data.action == "setupLoafProperties") {
            setupLoafProperties(data.properties)
        }
...
function setupLocations(locations, myHouses) {
    ...
    $(parent).append('<div class="loclabel" id="location" data-location="null" data-type="lab" data-label="Where would you like to start?"><p><span id="null">Vali alustamise asukoht</span></p></div>')
    
    setTimeout(function(){
        $(parent).append('<div class="location" id="location" data-location="current" data-type="current" data-label="Last Location"><p><span id="current-location">Viimane asukoht</span></p></div>');
...
function setupApps(apps) {
    ...
    $(parent).append('<div class="loclabel" id="location" data-location="null" data-type="lab" data-label="Vali Korter"><p><span id="null">Vali korter</span></p></div>')
...
function setupLoafProperties(properties) {
    if (properties == undefined) 
        return;
    var parent = $('.spawn-locations')
    setTimeout(() => {
        $.each(properties, (propertyId, propertyData) => {
            $(parent).append('<div class="location" id="location" data-location="'+propertyId+'" data-type="loaf_property" data-label="'+propertyData.label+'"><p><span id="'+propertyId+'">'+propertyData.label+'</span></p></div>')
        })
    }, 100)
}