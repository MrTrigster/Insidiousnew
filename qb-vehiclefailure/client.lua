	...
	if vehicle ~= nil and vehicle ~= 0 and engineHealth < 500 then
		...
		if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
			...
			if #(pos - vehpos) > 4.9 then
			  QBCore.Functions.Notify("Sa oled sõidukist liiga kaugel", "error")
			else
			  QBCore.Functions.Notify("Sa ei saa mootorit seestpoolt parandada", "error")
			end
		end
	else
		if vehicle == nil or vehicle == 0 then
		  	QBCore.Functions.Notify("Sa ei ole sõiduki juures", "error")
		else
		  	QBCore.Functions.Notify("Sõiduk on liiga terve", "error")
		end
	end
end)
...
function CleanVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("cleaning_vehicle", "Peseb autot...", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		QBCore.Functions.Notify("Auto pestud")
...
function RepairVehicleFull(vehicle)
	...
	QBCore.Functions.Progressbar("repair_vehicle", "Parandab autot...", math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		QBCore.Functions.Notify("Auto parandatud")
...
function RepairVehicle(vehicle)
	...
	QBCore.Functions.Progressbar("repair_vehicle", "Parandab autot...", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~g~" .. repairCfg.fixMessages[fixMessagePos] .. ", sõida nüüd töökotta")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			else
				notification("~r~Su auto on liiga katki")
			end
		else
		...
		notification("~y~Pead olema autos et seda parandada")
	end
end)

RegisterNetEvent('admin:repair')
AddEventHandler('admin:repair', function()
	if isPedDrivingAVehicle() then
		local ped = PlayerPedId()
		vehicle = GetVehiclePedIsIn(ped, false)
		...
		notification("Pead olema autos et seda parandada")
	end
end)
