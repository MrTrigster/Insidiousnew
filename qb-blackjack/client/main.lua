...
cfg.blackjackTables = {
    --[id] = {x,y,z,heading}
    [0] = {
        dealerPos = vector3(1003.45, 53.67, 69.43),
        dealerHeading = 232.69,
        tablePos = vector3(1004.184, 53.19242, 68.43275),
        tableHeading = 58.309322,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [1] = {
        dealerPos = vector3(1001.88, 59.87, 69.43),
        dealerHeading = 316.23,
        tablePos = vector3(1002.383, 60.50792, 68.43275),
        tableHeading = 143.30932,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [2] = {
        dealerPos = vector3(988.71, 46.48, 70.24),
        dealerHeading = 199.05,
        tablePos = vector3(989.0378, 45.7245, 69.23269),
        tableHeading = 23.30943,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01b"
    },
    [3] = {
        dealerPos = vector3(986.48, 42.02, 70.24),
        dealerHeading = 276.93,
        tablePos = vector3(987.2684, 42.20338, 69.23269),
        tableHeading = 103.30942,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01b"
    },
    [4] = {
        dealerPos = vector3(981.69, 62.69, 70.24),
        dealerHeading = 284.18,
        tablePos = vector3(982.4893, 62.90396, 69.23269),
        tableHeading = 103.30942,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01b"
    },
            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        drawNativeNotification("~INPUT_PICKUP~ Play Blackjack")
                    else 
                        drawNativeNotification("See tool on hõivatud")
                    end
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("Sinu käsi on ["..tostring(currentHand).."], dealeri käsi on ["..tostring(dealersHand).."]")
        ...
        if waitingForBetState then
            if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                ...
                if tonumber(tmpInput) then
                    ...
                    if tmpInput > 0 and tmpInput <= 5000 then
                        currentBetAmount = tmpInput
                    else
                        notify("Max panus on $5,000")
                    end
                end
            end
            if IsControlJustPressed(0, 201) then --Submit bet [Enter]
                --print("submitting bet")
                if tonumber(currentBetAmount) >= 0 and tonumber(currentBetAmount) <= 5000 then
            ...
            if IsControlPressed(0, 10) and GetGameTimer()-startTime > 250 then --Increase bet [pageup]
                if currentBetAmount ~= 5000 and currentBetAmount < 5000 then
            ...
            if IsControlPressed(0, 11) and GetGameTimer()-startTime > 250 then --Decrease bet [pagedown]
                if currentBetAmount >= 100 then 
		            ...
		            notify('Sa oled juba minimum panuse peal')
                end
            end            
        end
        Wait(0)
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    ClearHelp(true)
    drawNativeNotification("Tehke panused")
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            drawNativeNotification("Panuseid ei tehtud, round skipped")
        end
    end)
end)
        ...
        if closestChair == chairId then
            ...
            Citizen.CreateThread(function()
                ...
                if not standOrHitThisRound and sittingAtBlackjackTable then
                    ...
                    TriggerServerEvent("Blackjack:standBlackjack",globalGameId,globalNextCardCount)
                    declineCard()
                    drawNativeNotification("Sa ei teinud midagi, sinu eest valiti Stand")
                end
            end)
        else 
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Oota järgmist mängu...")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
