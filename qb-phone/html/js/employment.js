...
$(document).on('click', '.employment-btn-create-group', function(e){
    e.preventDefault();
    ClearInputNew()
...
$(document).on('click', '#employment-sbmit-for-create-group', function(e){
    ...
    if (Name != "" && pass != "" && pass2 != ""){
        if(pass == pass2){
            ...
            QB.Phone.Notifications.Add("fas fa-exclamation-circle", "System", "Vale Parool!")
        }
    }else{
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "System", "Lüngad on valesti täidetud!")
    }
});
...
function AddDIV(data){
    ...
    if(JSON.stringify(data) != "[]"){
        ...
        $(".employment-list").html("");
        var AddOption = '<div class="casino-text-clear">Pole Gruppe</div>'
...
$(document).on('click', '#employment-list-group', function(e){
    ...
    $.post('https://qb-phone/employment_CheckPlayerNames', JSON.stringify({
        id: id,
           ...
           var AddOption2 = '<p> </p>'+
           '<div class="phone-new-box-btn box-new-red" id="box-new-cancel">Sulge</div>'
