...
RegisterNetEvent('police:client:checkVin', function()
    local ped = PlayerPedId()
    local closestVeh, distance = QBCore.Functions.GetClosestVehicle()
    if distance < 2.5 then
        local plate = QBCore.Functions.GetPlate(closestVeh)
        TriggerServerEvent('police:server:checkVin', plate)
    end
end)
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            QBCore.Functions.Progressbar("robbing_player", "Röövib isikut...", math.random(5000, 7000), false, true, {
...
--         else
--             QBCore.Functions.Notify("Aeg peab olema rohkem kui 0", "error")
--         end
--     else
--         QBCore.Functions.Notify("Kedagi pole lähedal", "error")
--     end
...
RegisterNetEvent('police:client:BillPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(dialog['bill']) > 0 then
            ...
            QBCore.Functions.Notify("Summa peab olema rohkem kui 0", "error")
        end
    else
        QBCore.Functions.Notify("Kedagi pole lähedal", "error")
    end
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                ...
                QBCore.Functions.Notify("Sa ei saa autos isikut raudu panna", "error")
            end
        else
            QBCore.Functions.Notify("Kedagi pole lähedal", "error")
        end
        ...
        if player ~= -1 and distance < 1.5 then
            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                if result then 
                    ...
                    QBCore.Functions.Notify("Sul ei ole käeraudu", "error")
                end
            end, "handcuffs")
        else
            QBCore.Functions.Notify("Kedagi pole lähedal", "error")
...
RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff)
    ...
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            QBCore.Functions.Notify("Sa oled raudus")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            QBCore.Functions.Notify("Sa oled raudus aga saad kõndida")
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(ped)
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "Uncuff", 0.2)
        QBCore.Functions.Notify("Sult võeti rauad ära")
    end
end)
