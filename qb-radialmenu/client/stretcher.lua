...
AddEventHandler('i13-radialmenu:client:TakeStretcher', function()
    ...
    if Vehicle ~= 0 then
        ...
        QBCore.Functions.Notify("Sa ei ole Kiirabiautole lähedal", 'error')
    end
end)
        ...
        if StretcherObject ~= nil then
            ...
            if Distance <= 1.0 then
                if not IsAttached then
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, '~g~E~w~ - Lükka Alust')
                    ...
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, '~g~E~w~ - Lase Lahti')
                    if IsControlJustPressed(0, 51) then
                        DetachStretcher()
                        IsAttached = false
                    end
                ...
                if not IsLayingOnBed then
                    if not IsAttached then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z + 0.2, '~g~G~w~ - Heida Pikali')
            ...
            elseif Distance <= 2 then
                if not IsLayingOnBed then
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, 'Lükka Alust')
                else
                    if not IsAttached then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z + 0.2, '~g~G~w~ - Tõuse Püsti')
                        if IsControlJustPressed(0, 47) or IsDisabledControlJustPressed(0, 47) then
                            GetOffStretcher()
                        end
                    end
...
AddEventHandler('i13-radialmenu:client:Result', function(IsBusy, type)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            QBCore.Functions.Notify("Alus on juba kasutusel", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
