...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent("i13-houses:server:savedecorations", ClosestHouse, ObjectList)
	end
end
...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		QBCore.Functions.Notify("Kiirus on ".. tostring(speeds[curSpeed]))
	end
...
RegisterNetEvent('i13-houses:client:decorate', function()
	Citizen.Wait(500)
	if IsInside then
		if HasKey then
			...
			QBCore.Functions.Notify("Sul peavad maja võtmed olema", "error")
		end
	else
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    QBCore.Functions.TriggerCallback('i13-houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = not cursorEnabled
            SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				QBCore.Functions.Notify('Läksid liiga kaugele', 'error')
			end
		end
	end
end)