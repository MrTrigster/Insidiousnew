...
local menu = MenuV:CreateMenu(false, 'Admin Menu', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test')
local menu2 = MenuV:CreateMenu(false, 'Admin Options', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test1')
local menu4 = MenuV:CreateMenu(false, 'Online Players', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test3')
local menu5 = MenuV:CreateMenu(false, 'Manage Server', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test4')
local menu6 = MenuV:CreateMenu(false, 'Available Weather Options', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test5')
local menu7 = MenuV:CreateMenu(false, 'Dealer List', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test6')
local menu8 = MenuV:CreateMenu(false, 'Ban', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test7')
local menu9 = MenuV:CreateMenu(false, 'Kick', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test8')
local menu10 = MenuV:CreateMenu(false, 'Permissions', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test9')
local menu11 = MenuV:CreateMenu(false, 'Developer Options', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test10')
local menu12 = MenuV:CreateMenu(false, 'Vehicle Options', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test11')
local menu13 = MenuV:CreateMenu(false, 'Vehicle Categories', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test12')
local menu14 = MenuV:CreateMenu(false, 'Vehicle Models', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv', 'test13')

RegisterNetEvent('i13-admin:client:openMenu', function()
    MenuV:OpenMenu(menu)
end)
...
menu_button11:On("select",function()
    ...
    for k,v in ipairs(elements) do
        local menu_button14 = menu6:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('i13-weathersync:server:setWeather', selection.value)
...
RegisterNetEvent('i13-admin:client:ToggleCoords', function()
...
names_button:On('change', function()           
    TriggerEvent('i13-admin:client:toggleNames')
end)                                           
blips_button:On('change', function()           
    TriggerEvent('i13-admin:client:toggleBlips')
end)

-- Dealer List

local function OpenDealerMenu(dealer)
    local EditDealer = MenuV:CreateMenu(false, 'Edit Dealer ' .. dealer["name"], 'topright', 68, 68, 68, 'size-125', 'none', 'menuv')
...
local function OpenPermsMenu(permsply)
    QBCore.Functions.TriggerCallback('i13-admin:server:getrank', function(rank)
        if rank then
            ...
            local menu_button21 = menu10:AddButton({
                ...
                select = function(btn)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('i13-admin:server:setPermissions', permsply.id, selectedgroup)
    ...
    local menu_button18 = menu9:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('i13-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
    ...
    local menu_button17 = menu8:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('i13-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
...
local function OpenPlayerMenus(player)
    local Players = MenuV:CreateMenu(false, player.cid .. ' Options', 'topright', 68, 68, 68, 'size-125', 'none', 'menuv') -- Players Sub Menu
    Players:ClearItems()
    ...
    local elements = {
        ...
        [10] = {
            ...
            label = "Give Ped Menu",
            value = "ped",
            description = "Give the Ped menu to " .. player.cid
        },
        [11] = {
            icon = '🥾',
            label = "Kick",
        ...
        local menu_button10 = Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                    TriggerServerEvent('i13-admin:server:'..values, player)
...
menu_button13:On("select", function(item, value)
    TriggerServerEvent("i13-weathersync:server:setTime", value, value)
