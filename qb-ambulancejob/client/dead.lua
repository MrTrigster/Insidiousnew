...
function OnDeath()
    if not isDead then
        ...
        if isDead then
            ...
            -- TriggerServerEvent('hospital:server:ambulanceAlert', 'Isik Surnud')
	...
	while true do
		...
		if NetworkIsPlayerActive(player) then
            ...
            elseif IsEntityDead(playerPed) and InLaststand and not isDead then
                ...
                local killerName = killerId ~= -1 and GetPlayerName(killerId) .. " " .. "("..GetPlayerServerId(killerId)..")" or "Tema ise või Kohalik"
		...
		if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then
                    if deathTime > 0 then
                        DrawTxt(0.93, 1.44, 1.0,1.0,0.6, "RESPAWN: ~r~" .. math.ceil(deathTime) .. "~w~ SEKUNDIT", 255, 255, 255, 255)
                    else
                        DrawTxt(0.865, 1.44, 1.0, 1.0, 0.6, "~w~ HOIA ~r~[E] ("..hold..")~w~ ET RESPAWNIDA ~r~($"..Config.BillCost..")~w~", 255, 255, 255, 255)
                    end
                end
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Laststand.MinimumRevive then
                    DrawTxt(0.94, 1.44, 1.0, 1.0, 0.6, "SA JOOKSED VEREST TÜHJAKS: ~r~" .. math.ceil(LaststandTime) .. "~w~ SEKUNDIT", 255, 255, 255, 255)
                else
                    DrawTxt(0.845, 1.44, 1.0, 1.0, 0.6, "SA JOOKSED VEREST TÜHJAKS: ~r~" .. math.ceil(LaststandTime) .. "~w~ SEKUNDIT, SIND SAAB VEEL AIDATA", 255, 255, 255, 255)
                end
